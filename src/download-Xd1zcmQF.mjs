...
async function r(n) {
    let r = (n.req.param(`os`) ?? ``).toLowerCase(),
        i = `https://www.neatdownloadmanager.com`,
        a = `${i}/index.php`,
    ...
    return {
        ...
        item: o(`.p1`)
            ...
            .filter((e) => {
                e = o(e);
                let t = e.text().startsWith(`dmg`);
                return r === `` ? !0 : r === `macos` ? t : !t;
            })
            .map((e) => {
                e = o(e);
                let t = e.text(),
                    n = t.match(/\(ver (.*?)\)/)[1];
                return { title: `[${t.startsWith(`dmg`) ? `macOS` : `Windows`}] ${t}`, link: `${i}${e.prev().find(`a`).attr(`href`)}#${n}` };
            }),
    };
}
export { n as route };
