...
const n = { 0: `すべて`, 1: `お知らせ`, 2: `イベント`, 3: `メインテナンス`, 4: `重要` },
    r = (e) => {
        let t = [],
            n = [],
            r = [];
...
`);
    ...
    a = async (r) => {
        let { type: i = `0` } = r.req.param(),
            a = ((await e(`https://www.azurlane.jp/api/news/list`, { query: { type: i, index: 1, size: 15 } })).data?.rows || []).map((e) => ({
                title: e.title,
                description: e.content,
                link: `https://www.azurlane.jp/news/${e.id}`,
    ...
    o = {
        ...
        parameters: { server: 'game server (ISO 3166 two-letter country code, case-insensitive), only `JP` is supported for now', type: 'news type, see the table below, `0` by default' },
