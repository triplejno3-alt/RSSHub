...
const c = {
    ...
    radar: [{ source: [`zhiyou.smzdm.com/member/:uid/baoliao`] }],
    name: `用户爆料`,
    maintainers: [`nczitzk`],
    handler: l,
};
async function l(c) {
    ...
    let l = `https://zhiyou.smzdm.com/member/${c.req.param(`uid`)}/baoliao/`,
        u = s((await r(l, { headers: o() })).data),
        d = u(`.info-stuff-nickname`).text(),
        ...
        p = await Promise.all(
            f.map((e) =>
                t.tryGet(e.link, async () => {
                    ...
                    return ((e.description = t(`article.txt-detail`).html()), (e.pubDate = i(n(t(`.time`).first().text().trim().replace(`更新时间：`, ``)), 8)), (e.author = d), e);
                })
            )
        );
    return { title: `${d}的爆料 - 什么值得买`, link: l, item: p };
}
export { c as route };
