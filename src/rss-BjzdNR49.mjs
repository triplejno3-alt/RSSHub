...
import { t } from './utils-CHbYrdwc.mjs';
import n from 'rss-parser';
const r = {
    path: `/rss/:channel?`,
...
async function i(r) {
    let i = r.req.param(`channel`) ?? `rss-en-all`,
        a = await new n({ customFields: { item: [`dwsyn:contentID`] }, headers: { 'User-Agent': e.ua } }).parseURL(`https://rss.dw.com/rdf/${i}`),
        o = await t(
            a.items.map((e) => {
                ((e.id = e[`dwsyn:contentID`]), (e.pubDate = e.isoDate), (e.description = e.content));
                let t = new URL(e.link);
                return ((t.search = ``), (e.link = t.href), (e.type = t.pathname.slice(t.pathname.lastIndexOf(`/`) + 1).startsWith(`live-`) ? `liveblog` : `article`), e);
            })
        );
    return { title: a.title, link: a.link, description: a.description, item: o };
}
export { r as route };
