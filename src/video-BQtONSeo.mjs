...
| sanrenxing           | 价值三人行                   |
| yuandongli           | 中国源动力                   |
| pioneerzone          | 直击引领区                   |`};async function a(i){let a=i.req.param(`id`)??``,o;if(a){let e=await t({method:`get`,url:`${n}/api/ajax/getnavs`});for(let t of e.data.header.video)if(t.EnglishName===a||t.ChannelID===a){o={id:t.ChannelID,name:t.ChannelName,slug:t.EnglishName};break}}let s=`${n}/video${a?`/${o.slug}`:``}`,c=await r(`${n}/api/ajax/${a?`getlistbycid?cid=${o.id}`:`getjuhelist?action=video`}&page=1&pagesize=${i.req.query(`limit`)??30}`,e.tryGet);return{title:`第一财经 - ${o?.name??`视听`}`,link:s,item:c}}export{i as route};