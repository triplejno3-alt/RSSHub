...
import { t as o } from './description-BRmasTBj.mjs';
...
async function l(c) {
    let l = c.req.param(`user`) ?? ``,
        u = c.req.param(`id`) ?? ``,
        d = Number.parseInt(c.req.query(`limit`)) || 20;
    if (!a(l)) throw new i(`Invalid user`);
    let f = `https://${l}.itch.io/${u}/devlog`,
        p = s((await n({ method: `get`, url: f })).data),
        m = p(`.title`)
            .toArray()
            .slice(0, d)
            .map((e) => ((e = p(e)), { title: e.text(), link: e.attr(`href`), pubDate: r(t(e.text()), 8) }));
    return (
        (m = await Promise.all(
            m.map((t) =>
                e.tryGet(t.link, async () => {
                    let e = s((await n({ method: `get`, url: t.link })).data),
                        r = e(`script[type="application/ld+json"]:contains("@type":"BlogPosting")`),
                        i = JSON.parse(e(r).text());
                    return (
                        (t.author = i.author.name),
                        (t.pubDate = i.datePublished),
                        (t.description = o({
                            images: e(`.post_image`)
                                .toArray()
                                .map((t) => e(t).attr(`src`)),
                            description: e(`.post_body`).html(),
                        })),
                        t
                    );
                })
