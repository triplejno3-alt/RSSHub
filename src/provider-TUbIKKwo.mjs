...
import { a as n, i as r, o as i, t as a } from './utils-D_GlxMfh.mjs';
const o = {
...
async function s(o) {
    let { region: s, providerId: c } = o.req.param();
    if (![`hk`, `tw`].includes(s)) throw new t(`Unknown region: ${s}`);
    let l = o.req.query(`limit`) ? Number.parseInt(o.req.query(`limit`), 10) : 20,
        u = (await r(s, e.tryGet)).find((e) => e.key === c),
        d = i(s, await a(s, l, null, c)),
        f = await Promise.all(d.map((t) => n(t, e.tryGet)));
    return { title: `Yahoo 新聞 - ${u?.title ?? ``}`, link: u?.link ?? `https://${s}.news.yahoo.com`, image: `https://s.yimg.com/cv/apiv2/social/images/yahoo_default_logo-1200x1200.png`, item: f };
}
export { o as route };
