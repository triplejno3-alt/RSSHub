...
const i = `https://gs.njust.edu.cn`,
    a = {
        ...
        radar: [{ source: [`gs.njust.edu.cn/:type/list.htm`], target: `/gs/:type` }],
        name: `研究生院`,
...
async function o(a) {
    let o = i + (`/` + (a.req.param(`type`) ?? `sytzgg_4568`)) + `/list.htm`,
        s = r(await n(o)),
        c = `南京理工大学研究生院 -- ` + s(`title`).text(),
        l = s(`ul.news_ul`).find(`li`);
    return {
        title: c,
        link: o,
        item: await Promise.all(
            l.map(async (a, o) => {
                let c = s(o).find(`a`).attr(`href`),
                    l = ``;
                return (c.startsWith(`/`) && (l = r(await n(i + c))(`.wp_articlecontent`).html()), { title: s(o).find(`a`).attr(`title`).trim(), description: l, pubDate: t(e(s(o).find(`span`).text(), `YYYY-MM-DD`), 8), link: c });
            })
        ),
    };
}
