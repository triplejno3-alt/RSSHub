...
const i = {
    ...
    radar: [{ source: [`space.bilibili.com/:uid`], target: `/user/coin/:uid` }],
    name: `UP 主投币视频`,
    maintainers: [`DIYgod`],
    handler: a,
};
async function a(i) {
    let a = i.req.param(`uid`),
        ...
        { data: c, code: l, message: u } = (await t({ url: `https://api.bilibili.com/x/space/coin/video?vmid=${a}`, headers: { Referer: `https://space.bilibili.com/${a}/` } })).data;
    if (l) throw Error(u ?? l);
    return {
        title: `${s} 的 bilibili 投币视频`,
        link: `https://space.bilibili.com/${a}`,
        description: `${s} 的 bilibili 投币视频`,
        item: c.map((t) => ({
            title: t.title,
            description: n.renderUGCDescription(o, t.pic, t.desc, t.aid, void 0, t.bvid),
            pubDate: e(t.time * 1e3),
            link: t.time > n.bvidTime && t.bvid ? `https://www.bilibili.com/video/${t.bvid}` : `https://www.bilibili.com/video/av${t.aid}`,
            author: t.owner.name,
        })),
    };
}
