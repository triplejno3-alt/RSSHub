...
async function a(i) {
    let { id: a = `58af05dfb6b4300151760176d2aad0a04c275aaadbb1315039263f021f920dcd` } = i.req.param(),
        o = i.req.query(`limit`) ? Number.parseInt(i.req.query(`limit`), 10) : 15,
        s = `https://www.chinaisa.org.cn`,
        c = new URL(`gxportal/xfpt/portal/getColumnList`, s).href,
        l = new URL(`gxportal/xfpt/portal/viewArticleById`, s).href,
        u = new URL(`gxportal/xfgl/portal/list.html?columnId=${a}`, s).href,
        d = r((await e(c, { method: `POST`, headers: { 'Content-Type': `application/x-www-form-urlencoded` }, body: new URLSearchParams({ params: encodeURI(`{"columnId":"${a}"}`) }), parseResponse: JSON.parse })).articleListHtml),
        f = d(`ul.list li a`)
            .slice(0, o)
            .toArray()
            .map(
                (e) => (
                    (e = d(e)),
                    {
                        title: e.prop(`title`) ?? e.text(),
                        link: new URL(`gxportal/xfgl/portal/${e.prop(`href`)}`, s).href,
                        guid: e.prop(`href`).match(/articleId=(\w+)/)[1],
                        pubDate: n(e.parent().find(`span.times`).text().replaceAll(`[]`, ``)),
                    }
                )
            );
    f = await Promise.all(
            ...
            t.tryGet(i.link, async () => {
                let t = (
                        await e(l, {
                            ...
                            body: new URLSearchParams({ params: encodeURI(`{"articleId":"${i.guid}","columnId":"${a}"}`) }),
                            parseResponse: JSON.parse,
                        })
                    ).article_content,
                    o = r(t),
                    s = t.match(/文章来源：(.*?)日期：(\d+-\d+-\d+)/);
                return (
                    (i.title = o(`div.article_title`).contents().first().text() || i.title),
                    (i.description = o(`div.article_main`).html()),
                    (i.author = s[1].split(/&/)[0]),
                    (i.guid = `chinaisa-${i.guid}`),
                    (i.pubDate = n(s[2])),
                    i
                );
            })
    ...
    let p = d(`div.head-tit`).text();
    d = r(await e(u));
    let m = new URL(d(`link[rel="shortcut icon"]`).prop(`href`), s).href;
    return { item: f, title: `${d(`title`).text()} - ${p}`, link: u, description: d(`meta[name="description"]`).prop(`content`), language: `cn`, image: new URL(`img/logo.jpg`, s).href, icon: m, logo: m, subtitle: p, allowEmpty: !0 };
}
export { i as route };
