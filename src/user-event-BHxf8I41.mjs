...
const i = {
    ...
    radar: [{ source: [`github.com/:username`], target: `/user_event/:username` }],
    name: `User Event`,
    maintainers: [`mslxl`],
    handler: a,
};
async function a(n) {
    let i = n.req.param(`username`),
        a = n.req.param(`types`) || `all`,
        o = e.github && e.github.access_token,
        s = {};
    o && (s.Authorization = `token ${e.github.access_token}`);
    let c = r(a, (await t({ method: `get`, url: `https://api.github.com/users/${i}/events` + (o ? `` : `/public`), headers: s, searchParams: { per_page: 100 } })).data);
    return {
        title: `${i} GitHub User Feed - ${a === `all` ? `All Events` : `Events: ${a}`}`,
