...
const i = {
    ...
    features: {
        requireConfig: [{ name: `READWISE_ACCESS_TOKEN`, optional: !1, description: 'Visit `https://readwise.io/access_token` to get your access token.' }],
        requirePuppeteer: !1,
        antiCrawler: !1,
        supportBT: !1,
        supportPodcast: !1,
    ...
    radar: [{ source: [`read.readwise.io`], target: `/list` }],
...
async function a(i) {
    if (!t.readwise || !t.readwise.accessToken) throw new r(`Readwise access token is missing`);
    let a = `https://readwise.io/api/v3/list/?`,
        o,
        s;
    if (i.req.param(`routeParams`)) {
        let e = new URLSearchParams(i.req.param(`routeParams`)),
            t = e.get(`location`),
            n = e.get(`category`),
            r = e.get(`updatedAfter`);
        ((o = e.get(`tag`)), (s = e.get(`tagStrategy`) === `any` || e.get(`tagStrategy`) === `all` ? e.get(`tagStrategy`) : `any`), t && (a += `location=${t}&`), n && (a += `category=${n}&`), r && (a += `updatedAfter=${r}&`));
    }
    let c = [];
    async function l(n) {
        let r = await e(n, { headers: { Authorization: `Token ${t.readwise.accessToken}` } });
        (c.push(...r.results), r.nextPageCursor && (await l(a + `pageCursor=${r.nextPageCursor}`)));
    }
    return (
        await l(a),
        {
            ...
            item: c
                .filter((e) => {
                    ...
                    if (!t) return !1;
                    if (Array.isArray(o)) {
                        if (s === `any`) return o.some((e) => Object.values(t).some((t) => t.name === e));
                        if (s === `all`) return o.every((e) => Object.values(t).some((t) => t.name === e));
                    } else {
                        let e = o;
                        return Object.values(t).some((t) => t.name === e);
                    }
                    return !1;
                })
                .map((e) => ({ title: e.title, link: e.source_url, description: e.summary, pubDate: n(e.created_at), author: e.author })),
        }
    );
}
export { i as route };
