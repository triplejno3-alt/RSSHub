...
const i = {
        ...
        parameters: { gid: `活动所属组织ID，见下表`, typeId: `活动类别ID，见下表`, sortType: `排序方式，见下表`, token: `访问令牌，可通过示例中的令牌直接访问（会过期）` },
...
`,
        handler: async (i) => {
            let { gid: s, typeId: c, sortType: l, token: u } = i.req.param(),
                d = e.create({ baseURL: `https://jk.sicau.edu.cn/act/actInfo/v1.0.0`, headers: { 'x-access-token': u }, method: `post` }),
                f = async (e) => await d(`/getUserSchoolActList`, { query: { gid: o[s], typeId: a[c], sortType: l, page: e } }),
                p = await Promise.all([f(1), f(2)]);
            for (let e of p) if (e.code !== `0`) throw Error(e.message);
            let m = [...p[0].content, ...p[1].content].filter((e) => e.statusName !== `待发学时`).map((e) => ({ id: e.id, guid: e.id, title: e.title, image: e.logo }));
            return {
                title: `二课活动 - 四川农业大学`,
                ...
                item: await Promise.all(
                    m.map((e) =>
                        t.tryGet(String(e.id), async () => {
                            let { code: t, message: i, content: a } = await d(`/getActDetail?actId=${e.id}`);
                            if (t === `0`)
                                return (
                                    (e.author = a.groupName),
                                    (e.pubDate = r(n(a.startDate, `YYYY-MM-DD HH:mm:ss`), 8)),
                                    (e.category = [a.typeName, a.levelName]),
                                    (e.description = `<img src="${e.image}" alt="${e.title}" /><p style='white-space: pre-wrap'>${a.description}</p>`),
                                    e
                                );
                            throw Error(i);
                        })
