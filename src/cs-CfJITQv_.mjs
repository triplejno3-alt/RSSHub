...
| 3708     | 3709     | 3710     | 3725         | 3729     | 3732     | 3733     | 3740     | 3741     | 3742     | 3743     | 3744     | 3745     | 3746     | 3747     | 3748     | 3751     | 3752     | 3753     | 3755     | 3756           | 3759           | nlzfjs       | pyfa           | sjsyjxsfzx           | srxxgcddesdjs      | xxgcssscddhjs              |`,
    ...
    radar: [
        { source: [`cs.hrbust.edu.cn/:category/list.htm`], target: `/cs/:category` },
        { source: [`cs.hrbust.edu.cn`], target: `/cs` },
    ],
    view: i.Notifications,
};
async function s(i) {
    let o = `https://cs.hrbust.edu.cn/`,
        { category: s = 3709 } = i.req.param(),
        c = `${o}${s}/list.htm`,
        l = a(await e(c)),
        u = l(`li.col_title`).text(),
        d = l(`div.col_news_con li.news`)
            .toArray()
            .map((e) => {
                let t = l(e),
                    i = new URL(t.find(`a`).attr(`href`), o).href,
                    a = t.find(`span.news_meta`).text().trim(),
                    s = a ? r(n(a), 8) : null;
            ...
            d.map((n) =>
                t.tryGet(n.link, async () => {
                    ...
                    let t = a(await e(n.link)),
                        r = t(`div.wp_articlecontent`);
                    (r.find(`[style]`).removeAttr(`style`), r.find(`font`).contents().unwrap(), r.html(r.html()?.replaceAll(`&nbsp;`, ``)), r.find(`[align]`).removeAttr(`align`));
                    let i = t(`span.arti_publisher`).text().replace(`发布者：`, ``).trim();
                    return { title: n.title, link: n.link, pubDate: n.pubDate, description: r.html(), author: i };
                })
            )
        );
    return { title: `${u} - 哈尔滨理工大学计算机学院`, link: c, language: `zh-CN`, item: f };
}
export { o as route };
