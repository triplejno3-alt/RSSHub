...
const a = {
    ...
    handler: async (a) => {
        let o = `http://ai.ruc.edu.cn/${a.req.param(`category`)?.replaceAll(`-`, `/`) ?? `newslist/notice`}/`,
            s = o + `index.htm`,
            c = i(await e(s)),
            l = c(`title`).text(),
            u = c(`div.fr li`)
                .toArray()
                .map((e) => ((e = c(e)), { link: o + e.find(`a`).first().attr(`href`) }));
        return {
            title: l,
            link: s,
                ...
                u.map((a) =>
                    t.tryGet(a.link, async () => {
                        try {
                            ...
                            a.title = t(`title`).text();
                            let o = t(`div.tit`).find(`span`).first().text();
                            ((a.pubDate = r(n(/\d+-\d+-\d+/.exec(o)[0]), 8)),
                                (a.description = t(`div.fr`)
                                    .children()
                                    .slice(3)
                                    .toArray()
                                    .map((e) => t.html(e))
