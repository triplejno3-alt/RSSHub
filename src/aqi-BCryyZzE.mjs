...
async function r(n) {
    let r = n.req.param(`city`),
        i = n.req.param(`pollution`) || [],
        a = { so2: `so2`, no2: `no2`, co: `co`, o3: `O3`, pm25: `PM2.5`, pm10: `PM10` },
        o = (await t({ method: `get`, url: `http://aqicn.org/aqicn/json/android/${Number.isNaN(Number(r)) ? r : `@${r}`}/json` })).data,
        s =
            i.length === 0
                ? ``
                : i
                      .split(`,`)
                      .map((e) => {
                          let t = typeof o.historic[a[e]] == `object` ? o.historic[a[e]][Object.keys(o.historic[a[e]])[0]] : o.historic[a[e]][0];
                          return `${a[e].toUpperCase()}:<b>${t}</b><br>`;
                      })
                      .join(``);
    return {
        title: `${o.namena}AQI`,
        link: `https://aqicn.org/city/${o.ids.path}`,
        description: `${o.namena}AQI-aqicn.org`,
        item: [
            {
                title: `${o.namena}实时空气质量(AQI)${o.utimecn}`,
                description: `${o.infocn}<br>风力:<b>${o.cwind[0]}</b>级<br>AQI:<b>${o.aqi}</b><br>${s}<img src="${o.wgt}">`,
                pubDate: e(o.time * 1e3),
                guid: `${o.time}-${r}-${i}`,
                link: `https://aqicn.org/city/${o.ids.path}`,
            },
        ],
