...
async function s(o) {
    let s = o.req.param(`id`),
        l = `https://www.aeaweb.org`,
        u = `${l}/journals/${s}`,
        d = await n({ method: `get`, url: u }),
        f = i(d.data);
    (f(`.read-more`).remove(), (s = f(`input[name="journal"]`).attr(`value`)));
    let p = f(`.page-title`).text(),
        m = f(`.intro-copy`).text();
    ((d = await n({ method: `get`, url: `${l}/journals/search-results?journal=${s}&ArticleSearch[current]=1` })), (f = i(d.data)));
    let h = f(`h4.title a`)
        .toArray()
        .map((e) => ((e = f(e)), { link: `${l}${e.attr(`href`).split(`&`)[0]}` }));
    return (
        (h = await Promise.all(
            h.map((o) =>
                e.tryGet(o.link, async () => {
                    let e = i((await n({ method: `get`, url: o.link })).data);
                    return (
                        (o.doi = e(`meta[name="citation_doi"]`).attr(`content`)),
                        (o.guid = o.doi),
                        (o.title = e(`meta[name="citation_title"]`).attr(`content`)),
                        (o.author = e(`.author`)
                            .toArray()
                            .map((t) => e(t).text().trim())
                            .join(`, `)),
                        (o.pubDate = t(e(`meta[name="citation_publication_date"]`).attr(`content`), `YYYY/MM`)),
                        (o.description = a(
                            r(c, {
                                description: e(`meta[name="twitter:description"]`)
                                    .attr(`content`)
                                    .replace(/\(\w+ \d+\)( - )?/, ``),
                            })
                        )),
        ...
        { title: p, description: m, link: u, item: h, language: f(`html`).attr(`lang`) }
    );
}
const c = ({ description: e }) => (e ? r(`p`, { children: e }) : null);
export { o as route };
