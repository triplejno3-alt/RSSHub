...
const n = `https://www.jornada.com.mx`,
    r = {
        capital: `Capital`,
        cartones: `Cartones`,
...
async function o(a) {
    let o = a.req.param(`date`) === `today` || a.req.param(`date`) === void 0 ? i() : a.req.param(`date`),
        s = a.req.param(`category`),
        c = (await t(`${n}/jsonstorage/articles_${o}_.json`)).data,
        l = {};
    return (
        (l = s
            ? c.filter((e) => e.category === r[s]).map((t) => ({ title: t.title, description: t.content, pubDate: e(t.date), link: `${n}/${t.url}` }))
            : c.map((t) => ({ title: t.title, description: t.content, pubDate: e(t.date), link: `${n}/${t.url}` }))),
        { title: `La Jornada`, link: n, item: l }
    );
}
export { a as route };
