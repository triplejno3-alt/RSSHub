...
| 十五天       | shiwutian      |`,
    ...
    radar: [{ source: [`new.xianbao.fun`], target: `/` }],
};
async function r(n) {
    let r = n.req.param() || { category: `latest` },
        a = ``,
        o = r.category || `latest`,
        s = i.find((e) => e.id === o) || i.find((e) => e.id === `latest`),
        c = s.fullName,
        l = s.pushPath;
    a = o.endsWith(`xiaoshi`) || o.endsWith(`tian`) ? `${o}-hot.html` : o.endsWith(`hot`) ? `${o}.html` : o === `latest` ? `` : `category-${o}`;
    let u = await t(`http://new.xianbao.fun/plus/json/${l}.json`),
        d = JSON.parse(u.body),
        f;
    f = Array.isArray(d) ? d : d[Object.keys(d).find((e) => e.startsWith(`remen`))];
    let p = f.map((t) => ({ title: t.title, link: `http://new.xianbao.fun${t.url}`, description: t.content, pubDate: e(t.shijianchuo * 1e3), author: t.louzhu, category: t.catename }));
    return { title: `线板酷-${c}`, link: `http://new.xianbao.fun/${a}`, item: p };
}
const i = [
