...
async function l(c) {
    let l = c.req.param(`channel`),
        ...
        p = f(`div.spacer > div`)
            .toArray()
            .map((e) => ({
                title: f(e).find(`h3 > a.title`).text().trim(),
                author: f(e).find(`a.author`).text().trim(),
                link: new URL(f(e).find(`h3.title > a.title`).attr(`href`), o).href.replace(/(https:\/\/lvv2\.com.*?)\/title.*/, `$1`),
                pubDate: r(t(f(e).find(`a.dateline > time`).attr(`datetime`)), 8),
            }))
            .filter((e) => e.title !== ``),
        m = await Promise.all(
            p.map((t) =>
                e.tryGet(
                    ...
                    async () => (
                        (t.description =
                            new URL(t.link).hostname === `instant.lvv2.com`
                                ? await e.tryGet(t.link, async () =>
                                      a((await n(t.link)).data)(`#_tl_editor`)
                                          .html()
    ...
    return { title: `lvv2 - ${u ? s[l][u] : s[l]}`, link: d, item: m };
}
export { c as route };
