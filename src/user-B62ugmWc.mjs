...
function u(e, t) {
    return s(`https://app-api.pixiv.net/v1/user/illusts`, { headers: { ...a, Authorization: `Bearer ` + t }, searchParams: l.stringify({ user_id: e, filter: `for_ios` }) });
}
const d = {
    path: `/user/:id`,
    ...
    radar: [{ source: [`www.pixiv.net/users/:id`, `www.pixiv.net/en/users/:id`] }],
    name: `User Activity`,
    maintainers: [`DIYgod`],
    handler: f,
};
async function f(r) {
    ...
    let a = r.req.param(`id`),
        s = await o(t.tryGet);
    if (!s) throw new i(`pixiv not login`);
    let l = (await u(a, s)).data.illusts,
        d = l[0].user.name;
    return {
        title: `${d} 的 pixiv 动态`,
        link: `https://www.pixiv.net/users/${a}`,
        image: c.getProxiedImageUrl(l[0].user.profile_image_urls.medium),
        description: `${d} 的 pixiv 最新动态`,
        item: l.map((e) => {
            let t = c.getImgs(e);
            return {
                title: e.title,
                ...
                description: `${e.caption}<br><p>画师：${d} - 阅览数：${e.total_view} - 收藏数：${e.total_bookmarks}</p>${t.join(``)}`,
