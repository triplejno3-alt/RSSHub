...
const o = {
    ...
    parameters: { lang: 'Language, see below, `ko` by default', channel: 'RSS Feed Channel, see below, `news` by default' },
...
async function s(o) {
    let s = o.req.param(`lang`) ?? `ko`,
        c = o.req.param(`channel`) ?? `news`,
        l;
    switch (s) {
        case `ko`:
            l = `https://www.yna.co.kr/rss/${c}.xml`;
            break;
        default:
            l = `https://${s}.yna.co.kr/RSS/${c}.xml`;
            break;
    }
    let u = await i.parseURL(l),
        d = await Promise.all(
            u.items.map((i) =>
                e.tryGet(i.link, async () => {
                    i.pubDate = s === `ko` ? t(i.pubDate) : r(t(i.pubDate), 9);
                    ...
                    i.author =
                        ...
                        e(`.tit-name`)
                            ...
                            .map((t) => e(t).text())
                            .join(`, `);
                    let o = e(`article.story-news`);
                    return (o.find(`.related-group`).remove(), o.find(`.writer-zone01`).remove(), (i.description = o.html()), i);
                })
            )
        );
    return { title: u.title, link: u.link, description: u.description, language: u.language ?? s, item: d };
}
export { o as route };
