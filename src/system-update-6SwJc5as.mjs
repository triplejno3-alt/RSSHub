...
async function r() {
    let n = `https://www.nintendo.co.jp/support/switch/system_update/index.html`,
        r = t((await e(n)).data);
    return {
        title: `Nintendo Switch 本体更新情報`,
        link: n,
        item: r(`.c-heading-lv3`)
            .toArray()
            .slice(1, -2)
            .map((e) => {
                e = r(e);
                let t = e.text(),
                    i = /(\d+)年(\d+)月(\d+)日/.exec(t);
                return {
                    title: t,
                    author: `Nintendo`,
                    description: e
                        .nextUntil(`.c-heading-lv3`)
                        .toArray()
                        .map((e) => r(e).html()).join(`
`),
                    link: n,
                    guid: `${n}#${/(\d\.)+\d/.exec(t)[0]}`,
                    pubDate: new Date(Number.parseInt(i[1]), Number.parseInt(i[2]) - 1, Number.parseInt(i[3])),
                };
            }),
    };
}
