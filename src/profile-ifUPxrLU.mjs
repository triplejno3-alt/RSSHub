...
const t = {
    ...
    radar: [{ source: [`hey.xyz/u/:handle`], target: `/profile/:handle` }],
    name: `Lens Profile`,
    maintainers: [`DIYgod`],
    handler: n,
};
async function n(t) {
    let n = t.req.param(`handle`),
        r = (
            await e(`https://api-v2.lens.dev/`, {
                method: `POST`,
                json: {
                    operationName: `Profile`,
                    variables: { request: { forHandle: `lens/${n}` } },
        ...
        i = (
            await e(`https://api-v2.lens.dev/`, {
                ...
                json: {
                    ...
                    variables: { request: { limit: `TwentyFive`, where: { metadata: null, publicationTypes: [`POST`, `MIRROR`, `QUOTE`], from: [r.id] } } },
...
fragment PublicationMetadataMediaVideoFields on PublicationMetadataMediaVideo {
  ...
  __typename
    ...
    return {
        title: `${r.metadata.displayName} on Lens`,
        link: `https://hey.xyz/u/${n}`,
        item: i.map((e) => {
            let t = e.mirrorOn || e;
            return {
                title: t.metadata?.content || `No content`,
                description: `${e.mirrorOn ? `Mirrored: ` : ``}${t.metadata?.content || ``} ${t.metadata?.asset?.image?.optimized?.uri ? `<img src="${t.metadata?.asset?.image?.optimized?.uri}" />` : ``}`,
                link: `https://hey.xyz/posts/${e.id}`,
                pubDate: new Date(e.createdAt).toUTCString(),
                guid: e.id,
            };
        }),
    };
