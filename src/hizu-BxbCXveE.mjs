...
async function o(a) {
    let o = a.req.param(`column`) ?? `5dd92265e4b0bf88dd8c1175`,
        s = `https://www.hizh.cn`,
        c = (await n({ method: `get`, url: `${s}/channels/zjyapp/columns/${o}/stories.json` })).data.stories
            .slice(0, a.req.query(`limit`) ? Number.parseInt(a.req.query(`limit`)) : 50)
            .map((e) => ({ title: e.title, author: e.author, link: e.jsonUrl, category: e.columnName, pubDate: t(e.published) }));
    return (
        (c = await Promise.all(
            c.map((t) =>
                e.tryGet(t.link, async () => {
                    let e = r((await n({ method: `get`, url: t.link })).data.content);
                    return (e(`figure`).last().remove(), (t.description = e.html()), (t.link = `${s}/content.html?jsonUrl=${t.link}`), t);
                })
            )
        )),
        { title: `${i[o]} - 珠海网`, link: s, item: c }
    );
}
export { a as route };
