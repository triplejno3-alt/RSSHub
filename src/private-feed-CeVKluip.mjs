...
const i = {
    ...
    features: {
        requireConfig: [{ name: `GITHUB_ACCESS_TOKEN`, optional: !0, description: `GitHub access token to access private events` }],
        requirePuppeteer: !1,
        antiCrawler: !1,
        supportBT: !1,
        supportPodcast: !1,
    ...
    radar: [{ source: [`github.com/:user`], target: `/feed/:user` }],
    name: `User's Feed`,
    maintainers: [`RtYkk`],
    handler: a,
};
async function a(n) {
    let i = n.req.param(`user`),
        a = n.req.param(`types`) || `all`,
        o = e.github && e.github.access_token,
        s = {};
    o && (s.Authorization = `token ${e.github.access_token}`);
    let c = r(a, (await t({ method: `get`, url: `https://api.github.com/users/${i}/received_events`, headers: s, searchParams: { per_page: 100 } })).data),
        l = a === `all` ? `All Events` : `Events: ${a}`;
    return {
        title: `${i}'s GitHub ${o ? `Private Feed` : `Public Feed`} - ${l}`,
        link: `https://github.com/${i}`,
        description: `GitHub events received by ${i}${a === `all` ? `` : ` (filtered: ${a})`}${o ? ` - includes private events` : ` - public events only`}`,
        item: c,
    };
}
export { i as route };
