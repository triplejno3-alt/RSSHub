...
const i = {
    ...
    maintainers: [`TonyRL`],
    handler: a,
    radar: [{ source: [`dcfever.com/news/index.php`], target: `/news` }],
...
async function a(i) {
    let a = i.req.param(`type`),
        o = new URL(`${n}/news/index.php`, n);
    o.searchParams.append(`type`, a);
    let s = r(await e(o.href)),
        c = s(`.col-md-left .title a`)
            .toArray()
            .map((e) => ((e = s(e)), { title: e.text(), link: new URL(e.attr(`href`), o.href).href })),
        l = await Promise.all(c.map((e) => t(e)));
    return {
        title: `${s(`.channel_nav`)
            .contents()
            .filter((e, t) => t.nodeType === 3)
            .text()} - ${s(`head title`).text()}`,
        link: o.href,
