import"./ofetch-uhy-qh6X.mjs";import"./config-Cc-zZ5p-.mjs";import"./logger-_vmdpChp.mjs";import{t as e}from"./cache-DLkCV5c7.mjs";import"./helpers-C9wXLK0V.mjs";import{t}from"./parse-date-DjdQS_Nt.mjs";import{t as n}from"./got-CKQ7C9HX.mjs";import{Fragment as r,jsx as i,jsxs as a}from"hono/jsx/jsx-runtime";import{load as o}from"cheerio";import{renderToString as s}from"hono/jsx/dom/server";import{raw as c}from"hono/html";const l=({image:e,description:t})=>s(a(r,{children:[e?i(`figure`,{children:i(`img`,{src:e.src})}):null,t?i(r,{children:c(t)}):null]})),u={path:`/:category?`,categories:[`new-media`],example:`/thoughtco`,parameters:{category:`Category id, see below`},features:{requireConfig:!1,requirePuppeteer:!1,antiCrawler:!1,supportBT:!1,supportPodcast:!1,supportScihub:!1},name:`Category`,maintainers:[`nczitzk`],handler:d,description:`#### Science, Tech, Math
...
</details>`};async function d(r){let{category:i=``}=r.req.param(),a=r.req.query(`limit`)?Number.parseInt(r.req.query(`limit`),10):50,s=`https://www.thoughtco.com`,c=new URL(i,s).href,{data:u}=await n(c),d=o(u),f=d(`a[data-doc-id]`).slice(0,a).toArray().map(e=>(e=d(e),{title:e.find(`span.block-title`).text(),link:new URL(e.prop(`href`),s).href}));f=await Promise.all(f.map(r=>e.tryGet(r.link,async()=>{let{data:e}=await n(r.link),i=o(e);return i(`div.adslot`).remove(),i(`div.sources-and-citation, .mntl-figure-caption svg`).remove(),i(`div.figure-media`).each((e,t)=>{t=d(t);let n=t.find(`img`);t.replaceWith(l({image:{src:n.prop(`data-src`),width:n.prop(`width`),height:n.prop(`height`)}}))}),r.title=i(`meta[property="og:title"]`).prop(`content`),r.description=l({image:{src:i(`meta[property="og:image"]`).prop(`content`)},description:i(`div.article-content`).html()}),r.author=i(`meta[name="sailthru.author"]`).prop(`content`),r.category=[...new Set(i(`meta[name="parsely-tags"]`).prop(`content`)?.split(/,/).map(e=>e.trim()))],r.pubDate=t(e.match(/"datePublished": "(.*?)"/)[1]),r.updated=t(e.match(/"dateModified": "(.*?)"/)[1]),r})));let p=d(`meta[property="og:site_name"]`).prop(`content`),m=d(`meta[property="og:title"]`).prop(`content`),h=new URL(d(`link[rel="apple-touch-icon-precomposed"]`).prop(`href`),s).href;return{item:f,title:`${p}${m.startsWith(p)?``:` - ${m}`}`,link:c,description:d(`meta[property="og:description"]`).prop(`content`),language:d(`html`).prop(`lang`),image:d(`meta[property="og:image"]`).prop(`content`),icon:h,logo:h,subtitle:d(`meta[property="og:title"]`).prop(`content`),author:p}}export{u as route};