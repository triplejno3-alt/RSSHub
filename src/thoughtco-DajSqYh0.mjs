...
const l = ({ image: e, description: t }) => s(a(r, { children: [e ? i(`figure`, { children: i(`img`, { src: e.src }) }) : null, t ? i(r, { children: c(t) }) : null] })),
    u = {
        path: `/:category?`,
...
async function d(r) {
    let { category: i = `` } = r.req.param(),
        ...
        f = d(`a[data-doc-id]`)
            .slice(0, a)
            .toArray()
            .map((e) => ((e = d(e)), { title: e.find(`span.block-title`).text(), link: new URL(e.prop(`href`), s).href }));
    f = await Promise.all(
        f.map((r) =>
            e.tryGet(r.link, async () => {
                ...
                return (
                    i(`div.adslot`).remove(),
                    i(`div.sources-and-citation, .mntl-figure-caption svg`).remove(),
                    i(`div.figure-media`).each((e, t) => {
                        t = d(t);
                        let n = t.find(`img`);
                        t.replaceWith(l({ image: { src: n.prop(`data-src`), width: n.prop(`width`), height: n.prop(`height`) } }));
                    }),
                    (r.title = i(`meta[property="og:title"]`).prop(`content`)),
                    (r.description = l({ image: { src: i(`meta[property="og:image"]`).prop(`content`) }, description: i(`div.article-content`).html() })),
                    (r.author = i(`meta[name="sailthru.author"]`).prop(`content`)),
                    (r.category = [
                        ...new Set(
                            i(`meta[name="parsely-tags"]`)
                                .prop(`content`)
                                ?.split(/,/)
                                .map((e) => e.trim())
                        ),
                    ]),
                    (r.pubDate = t(e.match(/"datePublished": "(.*?)"/)[1])),
                    (r.updated = t(e.match(/"dateModified": "(.*?)"/)[1])),
                    r
                );
            })
        )
    );
    let p = d(`meta[property="og:site_name"]`).prop(`content`),
        m = d(`meta[property="og:title"]`).prop(`content`),
        h = new URL(d(`link[rel="apple-touch-icon-precomposed"]`).prop(`href`), s).href;
    return {
        item: f,
        title: `${p}${m.startsWith(p) ? `` : ` - ${m}`}`,
        link: c,
        description: d(`meta[property="og:description"]`).prop(`content`),
        language: d(`html`).prop(`lang`),
        image: d(`meta[property="og:image"]`).prop(`content`),
        icon: h,
        logo: h,
        subtitle: d(`meta[property="og:title"]`).prop(`content`),
        author: p,
    };
}
export { u as route };
