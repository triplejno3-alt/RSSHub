...
async function a(i) {
    let a = i.req.param(`category`) ?? ``,
        o = `https://www.newsmarket.com.tw${a ? `/blog/category/${a}` : ``}`,
        s = r((await n({ method: `get`, url: o })).data),
        c = s(`.title a`)
            .slice(0, i.req.query(`limit`) ? Number.parseInt(i.req.query(`limit`)) : 20)
            .toArray()
            .map((e) => ((e = s(e)), { title: e.text(), link: e.attr(`href`), pubDate: t() })),
        l = await Promise.all(
            c.map((i) =>
                e.tryGet(i.link, async () => {
                    let e = await n({ method: `get`, url: i.link }),
                        a = r(e.data);
                    return (
                        a(`figure img`).each(function () {
                            a(this)
                                .parent()
                                .html(`<img src="${a(this).attr(`data-src`)}">`);
                        }),
                        a(`.inline-post`).remove(),
                        (i.author = a(`.author-name`).text()),
