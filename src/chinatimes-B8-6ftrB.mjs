...
async function c(s) {
    let c = `https://www.chinatimes.com`,
        { category: l = `realtimenews` } = s.req.param(`category`),
        u = `${c}/${l}/?chdtv`,
        d = o(await e(u)),
        f = await a(),
        p = d(`.articlebox-compact`)
            .toArray()
            ...
            .map((e) => {
                ...
                return {
                    title: n.text().trim(),
                    link: `${c}${n.attr(`href`)}?chdtv`,
                    guid: `${c}${n.attr(`href`)}`,
                    pubDate: i(r(t.find(`time`).attr(`datetime`)), 8),
                    category: t
                        .find(`.category a`)
                        .toArray()
                        .map((e) => d(e).text().trim()),
                ...
                n.tryGet(e.link, async () => {
                    ...
                    return (
                        (e.category = [
                            ...new Set([
                                ...e.category,
                                ...i(`.article-hash-tag a`)
                                    .toArray()
                                    .map((e) => i(e).text().trim()),
                            ]),
                        ]),
                        i(`.ad, #donate-form-container, .promote-word, .google-news-promote, .article-hash-tag`).remove(),
                        (e.description = i(`.main-figure`).html() + i(`.article-body`).html()),
                        e
                    );
                })
            )
        );
    return (await f.close(), { title: d(`head title`).text(), description: d(`head meta[name="description"]`).attr(`content`), link: u, language: d(`html`).attr(`lang`), image: `${c}/images/2020/apple-touch-icon.png`, item: m });
}
export { s as route };
