...
async function o(a) {
    let o = a.req.param(`category`) ?? `news`,
        s = i[a.req.param(`language`) ?? `english`],
        c = r((await n({ method: `get`, url: `${s}/services/${o}/more.html` })).data),
        l = c(`.list__title`)
            .toArray()
            .map((e) => ((e = c(e)), { title: e.text(), link: `${s}${e.attr(`href`)}` }));
    return (
        (l = await Promise.all(
            l.map((i) =>
                e.tryGet(i.link, async () => {
                    ...
                    return (
                        (i.pubDate = t(e(`a[data-unixtime]`).attr(`data-unixtime`) * 1e3)),
                        (i.category = e(`.tag__text`)
                            .toArray()
                            .map((t) => e(t).text())),
                        e(`.article__meta, .article__title, .article__info, .article__quote-bg, .article__google-news, .article__footer, .m-buy, .photoview__ext-link`).remove(),
                        e(`div[data-type="article"]`).remove(),
        ...
        { title: `${o} - Sputnik News`, link: `${s}/${o}`, item: l }
    );
}
export { a as route };
