...
import { i as t, r as n, t as r } from './readable-social--hCfpJhv.mjs';
import { t as i } from './list-description-gNPp-xjC.mjs';
const a = {
    ...
    radar: [{ source: [`www.douban.com/subject_collection/:type`], target: `/list/:type` }],
    name: `豆瓣榜单与集合`,
...
async function o(a) {
    let o = a.req.param(`type`) || `subject_real_time_hotest`,
        s = Object.fromEntries(new URLSearchParams(a.req.param(`routeParams`))),
        c = r(void 0, t(s.playable), 0),
        l = r(void 0, n(s.score), 0),
        u = 0,
        d = [],
        f = ``,
        p = ``,
        m = null;
    for (; m === null || u < m; ) {
        let t = await e({ method: `get`, url: `https://m.douban.com/rexxar/api/v2/subject_collection/${o}/items?playable=${c}&start=${u}&count=50`, headers: { Referer: `https://m.douban.com/subject_collection/${o}` } });
        ((f = t.data.subject_collection.name), (p = t.data.subject_collection.description), (m = t.data.total));
        let n = t.data.subject_collection_items
            .filter((e) => (e.rating ? e.rating.value : 0) >= l)
            .map((e) => {
                let t = e.title;
                return {
                    title: t,
    ...
    return { title: `豆瓣 - ${f}`, link: `https://m.douban.com/subject_collection/${o}`, item: d, description: p };
}
export { a as route };
