...
const a = { 3892: `天下`, 3560: `四川`, 3909: `辟谣`, 3686: `国际`, 11: `云招考`, 3902: `30秒`, 3889: `拍客`, 3689: `体育`, 1: `国内`, 4002: `帮扶铁军`, 12: `文娱`, 46: `宽窄`, 4: `商业`, 21: `千面`, 17: `封面号` },
    o = {
        path: `/channel/:id?`,
...
async function s(o) {
    let s = o.req.param(`id`) ?? `3892`,
        c = `https://www.thecover.cn/channel_${s}`,
        l = i((await n({ method: `get`, url: c })).data),
        u = l(`a.link-to-article`)
            .toArray()
            .filter((e) => l(e).attr(`href`).startsWith(`/`))
            .map((e) => ({ link: `https://www.thecover.cn` + l(e).attr(`href`) })),
        d = await Promise.all(
            u.map((a) =>
                e.tryGet(a.link, async () => {
                    let e = i((await n({ method: `get`, url: a.link })).data);
                    ((a.title = e(`h1`, `.main-article`).text()), (a.description = e(`section.article-content`).html()));
                    let o = e(`span`, `.props-of-title`);
                    return ((a.author = o.eq(0).text()), (a.pubDate = r(t(o.eq(1).text(), `YYYY-MM-DD HH:mm`), 8)), a);
                })
            )
        );
    return {
