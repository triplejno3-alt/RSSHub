...
`,
            ...
            link: `https://ak.hypergryph.com/news/${e.cid}`,
            pubDate: r(e.displayTime, `X`),
        })),
    o = {
...
async function s(r) {
    let { group: o = `ALL` } = r.req.param(),
        s = await n.tryGet(
            `hypergryph:arknights:news`,
            async () => {
                let t = await e(`https://ak.hypergryph.com/news`),
                    n = i.load(t),
                    r = JSON.parse(
                        n(`script:contains("initialData")`)
                            .first()
                            .text()
                            .match(/self\.__next_f\.push\((.+)\)/)?.[1] ?? ``
                    );
                return JSON.parse(r[1].slice(2))[3].initialData;
            },
            t.cache.routeExpire,
            !1
        ),
        c = o === `ALL` ? Object.values(s).flatMap(({ list: e }) => a(e)) : a(s[o].list);
    return {
        title: `《明日方舟》游戏公告与新闻`,
        ...
        item: await Promise.all(
            c.map((t) =>
                n.tryGet(t.link, async () => {
                    let n = await e(t.link),
                        r = i.load(n)(`div > div > div > div > div > div > div:nth-child(4)`);
                    return ((t.description = r.length ? r.html() : t.description), t);
                })
            )
        ),
        language: `zh-cn`,
