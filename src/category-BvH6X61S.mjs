...
import { t as o } from './utils-DKggGkuR.mjs';
...
async function l(c) {
    let l = c.req.param(`category`),
        u = c.req.param(`column`),
        d = `https://${u}.caixin.com/${l}`;
    if (!a(u)) throw new i(`Invalid column`);
    let f = s((await n(d)).data),
        p = f(`head title`).text(),
        {
            data: { datas: m },
        } = await n(`https://gateway.caixin.com/api/extapi/homeInterface.jsp`, {
            searchParams: {
                subject: JSON.parse(
                    f(`script`)
                        .text()
                ...
                type: 0,
                count: c.req.query(`limit`) ? Number.parseInt(c.req.query(`limit`)) : 25,
                picdim: `_266_177`,
                start: 0,
            },
        }),
        h = m.map((e) => ({ title: e.desc, description: e.summ, link: e.link.replace(`http://`, `https://`), pubDate: r(t(e.time), 8), category: e.keyword.split(` `), audio: e.audioUrl, audio_image_url: e.pict.imgs[0].url }));
    return { title: p, link: d, description: `财新网 - 提供财经新闻及资讯服务`, item: await Promise.all(h.map((t) => e.tryGet(t.link, () => o(t)))) };
}
export { c as route };
