...
async function _(n) {
    let i = c[n.req.param(`category`) || `all`],
        s = await t({ method: `post`, url: `http://jw.scut.edu.cn/zhinan/jw/api/v2/findInformNotice.do?${r.stringify({ pageNo: 1, pageSize: 20, tag: i.tag })}`, headers: { Referer: a } });
    p(s.data);
    ...
    return { title: `华南理工大学教务处通知公告 - ` + i.title, link: `http://jw.scut.edu.cn/zhinan/cms/toPosts.do`, item: f };
}
export { g as route };
