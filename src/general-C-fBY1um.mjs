import { t as e } from './cache-DLkCV5c7.mjs';
import { t } from './parse-date-DjdQS_Nt.mjs';
import { r as n } from './common-utils-uYpL50sT.mjs';
import { t as r } from './got-CKQ7C9HX.mjs';
import { t as i } from './timezone-CrV-DT8S.mjs';
import { n as a } from './wechat-mp-HNgcLN2K.mjs';
import { Fragment as o, jsx as s, jsxs as c } from 'hono/jsx/jsx-runtime';
import { load as l } from 'cheerio';
import { renderToString as u } from 'hono/jsx/dom/server';
import { raw as d } from 'hono/html';
const f = (e) => (e ? e.split(`,`) : []),
    p = (e) => {
        let t = e.she_qi_items ?? {},
            n = f(t.zc_type_text),
            r = f(t.zc_type_icon),
            i = f(t.zc_key),
            a = f(t.company_type_text),
            l = f(t.industry_text),
            p = f(t.scale_text);
        return u(
            s(`div`, {
                class: `content`,
                children: c(`div`, {
                    class: `wrap`,
                    style: `max-width: 1200px;margin: auto;`,
                    children: [
                        s(`h3`, { class: `policy`, style: `text-align: center;font-size: 40px;font-weight: bold;color: #32638C;line-height: 66px;`, children: e.art_title }),
                        s(`p`, {
                            class: `department`,
                            style: `max-width: 506px;text-align: center;background: #3b87c7;font-size: 20px;font-weight: bold;color: #FFF;margin: 12px auto 36px;box-sizing: border-box;padding: 10px 15px;background: url(data:image/png;base64,) 0 0 no-repeat;background-size: 100% 100%;`,
                            children: e.pub_unite,
                        }),
                        c(`div`, {
                            class: `p_date`,
                            style: `text-align: center;margin: 0 0 36px 0;display: flex;justify-content: center;flex-wrap: wrap;`,
                            children: [
                                e.pub_time?.length ? c(`span`, { class: `pub_time`, style: `font-size: 20px;color: #32638C;width: 15em;margin: 0.5em;`, children: [`政策发布时间：`, e.pub_time] }) : null,
                                e.expiry_date?.length ? c(`span`, { class: `expiry_date`, style: `font-size: 20px;color: #32638C;width: 15em;margin: 0.5em;`, children: [`政策有效期：`, e.expiry_date] }) : null,
                            ],
                        }),
                        c(`div`, {
                            class: `smalltext`,
                            style: `position: relative;padding: 2em 0;max-width: 1044px;margin: auto;background: url(https://zcjd.cloud.gd.gov.cn/zcjd/TemplateOne/images/smalltext.png) center no-repeat;background-size: 100% 100%;`,
                            children: [
                                c(`div`, {
                                    class: `s_title`,
                                    style: `display: flex;flex-wrap: wrap;justify-content: space-between;padding: 2em 2em 0.5em;`,
                                    children: [
                                        s(`div`, { children: s(`span`, { class: `sLeft`, style: `line-height: 42px;font-size: 22px;border-left: 0.2em solid #3b87c7;padding-left: 0.3em;`, children: `政策原文` }) }),
                                        t.has_money
                                            ? s(`div`, {
                                                  class: `cIs she_qi`,
                                                  style: `border: 2px dashed #3B87C7;padding: 0 19px 0 12px;white-space: nowrap;`,
                                                  children: s(`span`, { style: `font-size: 20px;line-height: 42px;`, children: `本政策涉及资金支持` }),
                                              })
                                            : null,
                                    ],
                                }),
                                c(`div`, {
                                    class: `p_main`,
                                    style: `padding: 1em 2em;`,
                                    children: [
                                        s(`h3`, { style: `font-size: 22px;line-height: 36px;margin-bottom: 7px;text-align: center;`, children: e.zc_title }),
                                        s(`p`, { style: `font-size: 18px;line-height: 32px;text-indent: 2em;`, children: e.summary }),
                                    ],
                                }),
                                s(`a`, { href: e.link, class: `toOriginal`, style: `font-size: 20px;color: #2F6EA3;font-weight: bold;text-decoration: none;width: 90%;display: block;text-align: right;`, children: `>>阅读原文` }),
                            ],
                        }),
                        t.id
                            ? c(o, {
                                  children: [
                                      c(`div`, {
                                          class: `cBar she_qi`,
                                          style: `margin-top: 56px;`,
                                          children: [
                                              s(`h3`, {
                                                  class: `c_title`,
                                                  style: `text-align: center;background: url(data:image/png;base64,) center repeat-x;`,
                                                  children: s(`span`, { style: `display: inline-block;font-size: 36px;color: #32638C;font-weight: bold;background: #FFF;padding: 0 30px;line-height: 44px;`, children: `政策种类` }),
                                              }),
                                              s(`div`, {
                                                  class: `pTypeList`,
                                                  style: `text-align: center;`,
                                                  children: n.map((e, t) =>
                                                      c(`div`, {
                                                          class: `tIcon`,
                                                          style: `display: inline-block;vertical-align: middle;margin: 22px 35px 0;text-align: left;width: 180px;height: 86px;background: url(data:image/png;base64,) no-repeat;`,
                                                          children: [
                                                              s(`i`, {
                                                                  style: `display: inline-block;vertical-align: middle;width: 86px;line-height: 86px;text-align: center;font-size: 40px;color: #FFF;font-family: 'cmsicon';`,
                                                                  children: r[t],
                                                              }),
                                                              s(`span`, { style: `line-height: 86px;margin-left: 6px;font-size: 18px;color: #3B87C7;`, children: e }),
                                                          ],
                                                      })
                                                  ),
                                              }),
                                          ],
                                      }),
                                      c(`div`, {
                                          class: `cBar she_qi`,
                                          style: `margin-top: 56px;`,
                                          children: [
                                              s(`h3`, {
                                                  class: `c_title`,
                                                  style: `text-align: center;background: url(data:image/png;base64,) center repeat-x;`,
                                                  children: s(`span`, { style: `display: inline-block;font-size: 36px;color: #32638C;font-weight: bold;background: #FFF;padding: 0 30px;line-height: 44px;`, children: `政策关键词` }),
                                              }),
                                              s(`div`, {
                                                  class: `keywords`,
                                                  style: `text-align: center;margin-top: 46px;`,
                                                  children: i.map((e, t) =>
                                                      t % 2 == 0
                                                          ? s(`span`, {
                                                                style: `display: inline-block;vertical-align: middle;line-height: 48px;background: #BDDCF4;padding: 0 45px;font-size: 18px;color: #32638C;margin: 10px 4px 0;`,
                                                                children: e,
                                                            })
                                                          : s(`span`, {
                                                                class: `dBlue`,
                                                                style: `display: inline-block;vertical-align: middle;line-height: 48px;background: #3B87C7;padding: 0 45px;font-size: 18px;color: #FFF;margin: 10px 4px 0;`,
                                                                children: e,
                                                            })
                                                  ),
                                              }),
                                          ],
                                      }),
                                      c(`div`, {
                                          class: `cBar she_qi`,
                                          style: `margin-top: 56px;`,
                                          children: [
                                              s(`h3`, {
                                                  class: `c_title`,
                                                  style: `text-align: center;background: url(data:image/png;base64,) center repeat-x;`,
                                                  children: s(`span`, { style: `display: inline-block;font-size: 36px;color: #32638C;font-weight: bold;background: #FFF;padding: 0 30px;line-height: 44px;`, children: `适用群体` }),
                                              }),
                                              s(`div`, {
                                                  class: `zc-user-item`,
                                                  style: `display: flex;flex-direction: column;align-items: center;margin-top: 3em;`,
                                                  children: c(`div`, {
                                                      class: `zc-user-item_content zc-company-content`,
                                                      style: `position: relative;padding: 12px 12px 12px 6em;box-sizing: border-box;line-height: 20px;background: #f7fafd;border: 1px dotted #3b87c7;width: 92%;`,
                                                      children: [
                                                          s(`i`, {
                                                              class: `zc-user-item_icon`,
                                                              style: `position: absolute;left: -18px;margin-top: -22px;display: inline-block;line-height: 44px;font-size: 22px;color: #3b87c7;text-align: center;background: #bddcf4;padding: 0 0.5em;`,
                                                              children: `企业类型`,
                                                          }),
                                                          a.map((e) => s(`span`, { style: `color: #666;font-size: 18px;margin-left: 2em;font-weight: bold;line-height: 2em;`, children: e })),
                                                      ],
                                                  }),
                                              }),
                                              s(`div`, {
                                                  class: `zc-user-item`,
                                                  style: `display: flex;flex-direction: column;align-items: center;margin-top: 3em;`,
                                                  children: c(`div`, {
                                                      class: `zc-user-item_content zc-company-content`,
                                                      style: `position: relative;padding: 12px 12px 12px 6em;box-sizing: border-box;line-height: 20px;background: #f7fafd;border: 1px dotted #3b87c7;width: 92%;`,
                                                      children: [
                                                          s(`i`, {
                                                              class: `zc-user-item_icon`,
                                                              style: `position: absolute;left: -18px;margin-top: -22px;display: inline-block;line-height: 44px;font-size: 22px;color: #3b87c7;text-align: center;background: #bddcf4;padding: 0 0.5em;`,
                                                              children: `行业`,
                                                          }),
                                                          l.map((e) => s(`span`, { style: `color: #666;font-size: 18px;margin-left: 2em;font-weight: bold;line-height: 2em;`, children: e })),
                                                      ],
                                                  }),
                                              }),
                                              s(`div`, {
                                                  class: `zc-user-item`,
                                                  style: `display: flex;flex-direction: column;align-items: center;margin-top: 3em;`,
                                                  children: c(`div`, {
                                                      class: `zc-user-item_content zc-company-content`,
                                                      style: `position: relative;padding: 12px 12px 12px 6em;box-sizing: border-box;line-height: 20px;background: #f7fafd;border: 1px dotted #3b87c7;width: 92%;`,
                                                      children: [
                                                          s(`i`, {
                                                              class: `zc-user-item_icon`,
                                                              style: `position: absolute;left: -18px;margin-top: -22px;display: inline-block;line-height: 44px;font-size: 22px;color: #3b87c7;text-align: center;background: #bddcf4;padding: 0 0.5em;`,
                                                              children: `企业规模`,
                                                          }),
                                                          p.map((e) => s(`span`, { style: `color: #666;font-size: 18px;margin-left: 2em;font-weight: bold;line-height: 2em;`, children: e })),
                                                      ],
                                                  }),
                                              }),
                                          ],
                                      }),
                                  ],
                              })
                            : null,
                        e.jie_du_items?.length
                            ? s(`div`, {
                                  class: `interpret`,
                                  children: e.jie_du_items.map((e) =>
                                      c(`div`, {
                                          class: `iBar`,
                                          style: `position: relative;padding: 0px 15px 40px;border: 2px dashed #2F6EA3;margin: 80px auto 0;`,
                                          children: [
                                              s(`h3`, { class: `i_title`, style: `text-align: center;font-size: 36px;color: #32638C;`, children: e.jd_title }),
                                              s(`div`, {
                                                  class: `compare`,
                                                  children: s(`div`, {
                                                      class: `commonText`,
                                                      style: `border: 4px solid #3B87C7;background: #3B87C7;`,
                                                      children: s(`div`, {
                                                          class: `iNShow qaWhite`,
                                                          style: `padding: 20px;color: #FFFFFF;`,
                                                          children: s(`p`, { style: `font-size: 16px;line-height: 32px;text-indent: 2em;`, children: e.jd_content ? d(e.jd_content) : null }),
                                                      }),
                                                  }),
                                              }),
                                              e.attach_items?.length ? s(`div`, { class: `iList`, children: e.attach_items.map((e) => s(`img`, { style: `width: 100%;margin-top: 24px;`, src: e.file_url })) }) : null,
                                          ],
                                      })
                                  ),
                              })
                            : null,
                        e.wen_da_items?.length
                            ? s(`div`, {
                                  class: `qAnda`,
                                  children: s(`div`, {
                                      class: `swiper-container qaSwiper`,
                                      children: s(`div`, {
                                          class: `swiper-wrapper`,
                                          children: e.wen_da_items.map((e) =>
                                              s(`div`, {
                                                  class: `swiper-slide`,
                                                  children: c(`div`, {
                                                      class: `qSlide qaWhite`,
                                                      style: `margin-top: 2em;display: flex;flex-direction: column;align-items: center;`,
                                                      children: [
                                                          c(`h3`, { style: `margin: 0;padding: 1em;background: #3b87c7;color: #FFF;width: 100%;`, children: [`问：`, e.question] }),
                                                          c(`div`, { style: `border: 1px solid #3b87c7;box-sizing: border-box;padding: 1em;background: #ebf3f9;width: 100%;`, children: [`答：`, e.answer] }),
                                                      ],
                                                  }),
                                              })
                                          ),
                                      }),
                                  }),
                              })
                            : null,
                    ],
                }),
            })
        );
    },
    m = async (o, s) => {
        let c = n(s)
                .split(`/`)
                .filter((e) => e !== ``),
            [u, d] = c,
            f = o.pathstartat === void 0 ? 0 : o.pathstartat,
            m = `http://` + d + `.` + u + `.gov.cn`;
        for (let e = 0; e < f; e++) {
            let t = c[e + 2];
            m = m + `/` + t;
        }
        let h = o.defaultPath,
            g = o.name_element,
            _ = o.name_match_type,
            v = o.name_match,
            y = o.name_join,
            b = o.list_element,
            x = o.list_include;
        if ((b === void 0 && ((b = `a[href*="content"]`), (x = `site`)), x === `site`)) {
            b = b.split(`,`).filter((e) => e !== ``);
            for (let e = 0; e < b.length; e++) b[e] += `[href*="` + m.slice(7) + `"]`;
            b = b.join(`,`);
        }
        let S = o.title_element,
            C = o.title_match,
            w = o.author_element,
            T = o.author_match,
            E = o.authorisme,
            D = o.description_element,
            O = o.pubDate_element,
            k = o.pubDate_match,
            A = o.pubDate_format;
        c.splice(0, 2 + f);
        let j = c.join(`/`);
        j = j === `` ? h : j.endsWith(`/`) ? j : j + `/`;
        let M = `${m}/${j}`,
            N = ``,
            P = ``,
            F = ``;
        if (j.startsWith(`gkmlpt`)) {
            ((S = void 0), (C = void 0), (D = `div[class="article-content"]`), (O = void 0), (k = void 0), (A = void 0));
            let e = await r(`${m}/gkmlpt/api/all/0`);
            ((P = E + `政府信息公开平台`), (F = e.data.articles.filter((e) => e.url.includes(`content`))));
        } else {
            let e = (await r(M)).data;
            switch (((N = l(e)), _)) {
                case `name`:
                    P = g;
                    break;
                case `meta`:
                    g = g.split(`,`).filter((e) => e !== ``);
                    for (let e = 0; e < g.length; e++) g[e] = N(`meta[name="` + g[e].trim() + `"]`).attr(`content`);
                    P = g.join(y);
                    break;
                case `element`:
                    g = g.split(`,`).filter((e) => e !== ``);
                    for (let e = 0; e < g.length; e++) g[e] = N(g[e].trim()).text().match(v)[1];
                    P = g.join(y);
                    break;
                default:
                    P = N(`head title`).text();
                    break;
            }
            F = N(b);
        }
        let I = F.map((e, t) => {
                let n = ``;
                return (j.startsWith(`gkmlpt`) ? (n = e.url) : ((n = N(t).attr(`href`)), n.startsWith(`http`) || (n = n.startsWith(`/`) ? `${m}${n}` : `${m}/${n}`)), n);
            }),
            L = await Promise.all(
                I.map((n) => {
                    let o = new URL(n);
                    return o.pathname === `/zcjdpt`
                        ? e.tryGet(n, async () => {
                              let e = (await r(`https://zcjd.cloud.gd.gov.cn/api/home/article` + o.search)).data.data;
                              for (let t of e.jie_du_items) t.jd_content = t.jd_content.replaceAll(/((\n {4})|(\n))/g, `</p><p style="font-size: 16px;line-height: 32px;text-indent: 2em;">`);
                              return { link: n, title: e.art_title, description: p(e), pubDate: i(t(e.pub_time), 8), author: /(本|本网|本站)/.test(e.pub_unite) ? E : e.pub_unite };
                          })
                        : o.host === `mp.weixin.qq.com`
                          ? a({ link: n })
                          : e.tryGet(n, async () => {
                                let { data: e } = await r(n),
                                    a = l(e),
                                    o = ``;
                                o = w === void 0 ? a(`meta[name="ContentSource"]`).attr(`content`) : a(w).text().trim().match(T)[1].trim().replaceAll(/(-*$)/g, ``);
                                let s = ``;
                                s = O === void 0 ? a(`meta[name="PubDate"]`).attr(`content`) : a(O).text().trim().match(k)[1].trim().replaceAll(/(-*$)/g, ``);
                                let c = ``;
                                c = S === void 0 ? a(`meta[name="ArticleTitle"]`).attr(`content`) : a(S).text().trim().match(C)[1];
                                let u = D.split(`,`).filter((e) => e !== ``);
                                for (let e = 0; e < u.length; e++) u[e] = a(u[e].trim()).html();
                                let d = u.join(``);
                                return { link: n, title: c, description: d, pubDate: i(t(s, A), 8), author: /本|本网|本站/.test(o) ? E : o };
                            });
                })
            );
        return { title: P, link: M, item: L };
    };
export { m as t };
