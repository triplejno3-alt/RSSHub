...
const r = `http://physics.zju.edu.cn`,
    i = new Map([
...
async function o(a) {
    let o = Number.parseInt(a.req.param(`type`)),
        s = i.get(o).id,
        c = n((await t({ method: `get`, url: `${r}/${s}/list.htm` })).data),
        l = c(`#arthd li`)
            .toArray()
            .map((t) => ((t = c(t)), { title: t.find(`a`).attr(`title`), pubDate: e(t.find(`.art-date`).text()), link: `http://physics.zju.edu.cn/${t.find(`a`).attr(`href`)}` }));
