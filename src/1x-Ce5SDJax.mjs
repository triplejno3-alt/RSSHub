...
const s = async (s) => {
        let { category: c = `latest/awarded` } = s.req.param(),
            l = s.req.query(`limit`) ? Number.parseInt(s.req.query(`limit`), 10) : 30,
            u = `https://1x.com`,
            d = new URL(`gallery/${c}`, u).href,
            { data: f } = await e(d),
            p = i(f),
            m = p(`html`).prop(`lang`),
            h = new URL(`backend/lm2.php?style=normal&mode=${p(`input#lm_mode`).prop(`value`)}`, u).href,
            { data: g } = await e(h),
            _ = i(g)(`div.photos-feed-item`)
                .slice(0, l)
                .toArray()
                .map((e) => {
                    e = p(e);
                    let i = e.find(`span.photos-feed-data-title`).first().text() || `Untitled`,
                        s = e.find(`img`).prop(`src`),
                        c = e.find(`span.photos-feed-data-name`).first().text(),
                        l = `${i} by ${c}`,
                        d = a(r(t, { children: [s ? n(`figure`, { children: n(`img`, { src: s, alt: i }) }) : null, l ? n(t, { children: o(l) }) : null] })),
                        f = e.find(`img[id]`).prop(`id`).split(/-/).pop(),
                        h = `1x-${f}`;
                    return {
                        title: i,
                        description: d,
                        link: new URL(`photo/${f}`, u).href,
                        author: c,
                        guid: h,
                        id: h,
                        content: { html: d, text: l },
                        image: s,
                        banner: s,
                        language: m,
                        enclosure_url: s,
                        enclosure_type: s ? `image/${s.split(/\./).pop()}` : void 0,
                        enclosure_title: i,
                    };
                }),
            v = new URL(p(`img.themedlogo`).prop(`src`), u).href;
        return { title: p(`title`).text(), description: p(`meta[name="description"]`).prop(`content`), link: d, item: _, allowEmpty: !0, image: v, author: p(`meta[property="og:site_name"]`).prop(`content`), language: m };
    },
    c = {
        path: `/:category{.+}?`,
        name: `Gallery`,
        ...
        radar: [{ source: [`/gallery/:category*`, `/photos/:category*`], target: `/1x/:category` }],
    };
