...
const i = async (i) => {
        let { category: a = `sylm/xyxw` } = i.req.param(),
            o = i.req.query(`limit`) ? Number.parseInt(i.req.query(`limit`), 10) : 11,
            s = `mse.hust.edu.cn`,
            c = `https://${s}`,
            l = new URL(`${a}.htm`, c).href,
            { data: u } = await n(l),
            d = r(u),
            f = d(`html`).prop(`lang`),
            p = d(`div.list ul li`)
                ...
                .map((e) => {
                    e = d(e);
                    let n = e.find(`a`);
                    return { title: n.text(), pubDate: t(e.find(`span.time`).text()), link: new URL(n.prop(`href`), l).href, language: f };
                });
        p = await Promise.all(
            p.map((i) =>
                e.tryGet(i.link, async () => {
                    if (!i.link.includes(s)) return i;
                    let { data: e } = await n(i.link),
                        a = r(e),
                        o = a(`h1.article_title`).text() || a(`div.article_title`).text() || i.title,
                        c = a(`div.v_news_content`).html();
                    for (let e of a(`div.article_data`).contents().toArray()) {
                        let n = a(e).text();
                        if (!i.pubDate && n.startsWith(`发布`)) {
                            let e = n.split(/：/)?.pop();
                            i.pubDate = e ? t(e) : i.pubDate;
                        } else ((!i.author && n.startsWith(`作者`)) || (!i.author && n.startsWith(`编辑`))) && (i.author = n.split(/：/)?.pop() ?? void 0);
                    }
                    return ((i.title = o), (i.description = c), (i.content = { html: c, text: a(`div.v_news_content`).text() }), (i.language = f), i);
                })
            )
        );
        let m = d(`meta[name="keywords"]`).prop(`content`)?.replaceAll(`,`, ` - `) ?? d(`title`).text(),
            h = new URL(d(`div.logo img`).prop(`src`), c).href;
        return { title: m, description: m.split(/-/).pop()?.trim(), link: l, item: p, allowEmpty: !0, image: h, author: m.split(/-/)[0]?.trim(), language: f };
    },
    a = {
        path: `/mse/:category{.+}?`,
        name: `机械科学与工程学院`,
...
| [xyzl/xydt](https://rsshub.app/hust/mse/xyzl/xydt) | [xyzl/jcxy](https://rsshub.app/hust/mse/xyzl/jcxy) | [xyzl/xyml](https://rsshub.app/hust/mse/xyzl/xyml) | [xyzl/xyzp](https://rsshub.app/hust/mse/xyzl/xyzp) | [xyzl/fwxy](https://rsshub.app/hust/mse/xyzl/fwxy) | [xyzl/cyxz](https://rsshub.app/hust/mse/xyzl/cyxz) |
    ...
    `,
        ...
        radar: [
            ...
            { title: `首页栏目 - 学院新闻`, source: [`mse.hust.edu.cn/xyxw.htm`], target: `/mse/xyxw` },
            { title: `首页栏目 - 通知公告`, source: [`mse.hust.edu.cn/tzgg.htm`], target: `/mse/tzgg` },
            { title: `首页栏目 - 招生招聘`, source: [`mse.hust.edu.cn/zszp.htm`], target: `/mse/zszp` },
            { title: `首页栏目 - 媒体聚焦`, source: [`mse.hust.edu.cn/mtjj.htm`], target: `/mse/mtjj` },
            { title: `首页栏目 - 期刊动态`, source: [`mse.hust.edu.cn/qkdt.htm`], target: `/mse/qkdt` },
            { title: `首页栏目 - 学术活动`, source: [`mse.hust.edu.cn/xshd.htm`], target: `/mse/xshd` },
            { title: `首页栏目 - 师生天地`, source: [`mse.hust.edu.cn/sstd.htm`], target: `/mse/sstd` },
            { title: `首页栏目 - STAR风采`, source: [`mse.hust.edu.cn/STARfc.htm`], target: `/mse/STARfc` },
            { title: `理论学习 - 党务动态`, source: [`mse.hust.edu.cn/llxx1/dwdt/djxw.htm`], target: `/mse/llxx1/dwdt/djxw` },
            { title: `理论学习 - 共青团`, source: [`mse.hust.edu.cn/llxx1/gqt/xwdt.htm`], target: `/mse/llxx1/gqt/xwdt` },
            { title: `理论学习 - 工会组织`, source: [`mse.hust.edu.cn/llxx1/ghzz/xwgg.htm`], target: `/mse/llxx1/ghzz/xwgg` },
            { title: `理论学习 - 学习参考`, source: [`mse.hust.edu.cn/llxx1/xxck.htm`], target: `/mse/llxx1/xxck` },
            { title: `理论学习 - 资料汇编`, source: [`mse.hust.edu.cn/llxx1/zlhb.htm`], target: `/mse/llxx1/zlhb` },
            { title: `理论学习 - 其他群团`, source: [`mse.hust.edu.cn/llxx1/ghzz1/lmmc.htm`], target: `/mse/llxx1/ghzz1/lmmc` },
            { title: `师资队伍 - 人才招聘`, source: [`mse.hust.edu.cn/szdw/rczp.htm`], target: `/mse/szdw/rczp` },
            { title: `师资队伍 - 常用下载`, source: [`mse.hust.edu.cn/szdw/cyxz.htm`], target: `/mse/szdw/cyxz` },
            { title: `人才培养 - 本科生教育`, source: [`mse.hust.edu.cn/rcpy/bksjy.htm`], target: `/mse/rcpy/bksjy` },
            { title: `人才培养 - 研究生教育`, source: [`mse.hust.edu.cn/rcpy/yjsjy.htm`], target: `/mse/rcpy/yjsjy` },
            { title: `人才培养 - 学生工作`, source: [`mse.hust.edu.cn/rcpy/xsg_z.htm`], target: `/mse/rcpy/xsg_z` },
            { title: `人才培养 - 机械创新基地`, source: [`mse.hust.edu.cn/rcpy/jxcxjd.htm`], target: `/mse/rcpy/jxcxjd` },
            { title: `人才培养 - 常用下载`, source: [`mse.hust.edu.cn/rcpy/cyxz.htm`], target: `/mse/rcpy/cyxz` },
            { title: `科学研究 - 科研动态`, source: [`mse.hust.edu.cn/kxyj/kydt.htm`], target: `/mse/kxyj/kydt` },
            { title: `科学研究 - 安全管理`, source: [`mse.hust.edu.cn/kxyj/aqgl.htm`], target: `/mse/kxyj/aqgl` },
            { title: `科学研究 - 设备开放`, source: [`mse.hust.edu.cn/kxyj/sbkf.htm`], target: `/mse/kxyj/sbkf` },
            { title: `科学研究 - 科研成果`, source: [`mse.hust.edu.cn/kxyj/kycg.htm`], target: `/mse/kxyj/kycg` },
            { title: `科学研究 - 常用下载`, source: [`mse.hust.edu.cn/kxyj/cyxz.htm`], target: `/mse/kxyj/cyxz` },
            { title: `社会服务 - 驻外研究院`, source: [`mse.hust.edu.cn/shfw/zwyjy.htm`], target: `/mse/shfw/zwyjy` },
            { title: `社会服务 - 产业公司`, source: [`mse.hust.edu.cn/shfw/cygs.htm`], target: `/mse/shfw/cygs` },
            { title: `合作交流 - 专家来访`, source: [`mse.hust.edu.cn/hzjl/zjlf.htm`], target: `/mse/hzjl/zjlf` },
            { title: `合作交流 - 师生出访`, source: [`mse.hust.edu.cn/hzjl/sscf.htm`], target: `/mse/hzjl/sscf` },
            { title: `合作交流 - 项目合作`, source: [`mse.hust.edu.cn/hzjl/xmhz.htm`], target: `/mse/hzjl/xmhz` },
            { title: `合作交流 - 国际会议`, source: [`mse.hust.edu.cn/hzjl/gjhy.htm`], target: `/mse/hzjl/gjhy` },
            { title: `合作交流 - 常用下载`, source: [`mse.hust.edu.cn/hzjl/cyxz.htm`], target: `/mse/hzjl/cyxz` },
            { title: `校友专栏 - 校友动态`, source: [`mse.hust.edu.cn/xyzl/xydt.htm`], target: `/mse/xyzl/xydt` },
            { title: `校友专栏 - 杰出校友`, source: [`mse.hust.edu.cn/xyzl/jcxy.htm`], target: `/mse/xyzl/jcxy` },
            { title: `校友专栏 - 校友名录`, source: [`mse.hust.edu.cn/xyzl/xyml.htm`], target: `/mse/xyzl/xyml` },
            { title: `校友专栏 - 校友照片`, source: [`mse.hust.edu.cn/xyzl/xyzp.htm`], target: `/mse/xyzl/xyzp` },
            { title: `校友专栏 - 服务校友`, source: [`mse.hust.edu.cn/xyzl/fwxy.htm`], target: `/mse/xyzl/fwxy` },
            { title: `校友专栏 - 常用下载`, source: [`mse.hust.edu.cn/xyzl/cyxz.htm`], target: `/mse/xyzl/cyxz` },
            { title: `理论学习 - 党务动态`, source: [`mse.hust.edu.cn/llxx1/dwdt/djxw.htm`], target: `/mse/llxx1/dwdt/djxw` },
            { title: `理论学习 - 共青团`, source: [`mse.hust.edu.cn/llxx1/gqt/xwdt.htm`], target: `/mse/llxx1/gqt/xwdt` },
            { title: `理论学习 - 工会组织`, source: [`mse.hust.edu.cn/llxx1/ghzz/xwgg.htm`], target: `/mse/llxx1/ghzz/xwgg` },
            { title: `理论学习 - 学习参考`, source: [`mse.hust.edu.cn/llxx1/xxck.htm`], target: `/mse/llxx1/xxck` },
