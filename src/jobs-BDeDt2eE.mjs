...
import { a as r, d as i, f as a, i as o, l as s, n as c, o as l, r as u, u as d } from './utils-Bk8Ks1dv.mjs';
const f = {
    ...
    radar: [
        {
            ...
            target: (e, t) => {
                let n = new URLSearchParams(new URL(t).search),
                    r = a(n.get(`f_JT`)),
                    i = a(n.get(`f_E`)),
                    o = encodeURIComponent(n.get(`keywords`) || ``),
                    s = new URLSearchParams();
                for (let [e, t] of n.entries()) [`f_JT`, `f_E`, `keywords`].includes(e) || s.append(e, t);
                return `/linkedin/jobs/${r}/${i}/${o}/?${s.toString()}`;
            },
        },
    ],
    name: `Jobs`,
...
async function p(n) {
    let a = d(n.req.param(`job_types`), o),
        f = d(n.req.param(`exp_levels`), c),
        p = new URLSearchParams(n.req.param(`routeParams`)),
        m = new URL(`/jobs-guest/jobs/api/seeMoreJobPostings/search`, `https://www.linkedin.com/`);
    (m.searchParams.append(l, n.req.param(`keywords`) || ``), m.searchParams.append(r, a), m.searchParams.append(u, f));
    for (let [e, t] of p) m.searchParams.has(e) || m.searchParams.append(e, t);
    m = m.toString();
    let h = s(await e(m)),
        g = i(n.req.param(`job_types`), o),
        _ = i(n.req.param(`exp_levels`), c);
    return {
        title: `LinkedIn Job Listing` + (g ? ` | Job Types: ${g}` : ``) + (_ ? ` | Experience Levels: ${_}` : ``) + (n.req.param(`keywords`) ? ` | Keywords: ${n.req.param(`keywords`)}` : ``),
        link: m,
        description: `This feed gets LinkedIn job posts`,
        item: h.map((e) => ({ title: `${e.company} is hiring ${e.title}`, description: `Title: ${e.title} | Company: ${e.company} | Location: ${e.location} `, pubDate: t(e.pubDate), link: e.link })),
    };
}
export { f as route };
