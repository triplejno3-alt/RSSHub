...
| 339      | 340      | 342      | 346      | 351      | 353      | 354      | 355      | 442      | 443      | 444      | 445      | 2106         | 2332     | 2333     | 2334     | 2335     | 2336     | 2730             | 2855     | 2857     | 2859     | 3271     | 3508           | 3519                 | 3981     | 4057     | 4058     | 4059   | 4060     | 4061     | 4062     |`,
    ...
    radar: [{ source: [`jwzx.hrbust.edu.cn/homepage/index.do`], target: `/jwzx` }],
    view: i.Notifications,
};
async function s(i) {
    let o = `http://jwzx.hrbust.edu.cn/homepage/`,
        { type: s = 354, page: c = 12 } = i.req.param(),
        l = o + `infoArticleList.do?columnId=` + s + `&pagingNumberPer=` + c,
        u = a(await e(l)),
        d = u(`.columnTitle .wow span`).text().trim(),
        f = u(`div.articleList li`)
            .toArray()
            .map((e) => {
                let t = u(e),
                    i = new URL(t.find(`a`).attr(`href`), o).href;
                return { title: t.find(`a`).text().trim(), link: i, pubDate: r(n(t.find(`span`).text().trim()), 8) };
            }),
        p = await Promise.all(
            f.map((n) =>
                t.tryGet(n.link, async () => {
                    if (!n.link.startsWith(o)) return ((n.description = `本文需跳转，请点击原文链接后阅读`), n);
                    let t = a(await e(n.link))(`div.body`);
                    return (
                        ...
                        n.description === null && (n.description = `解析正文失败`),
                        n
                    );
                })
            )
        );
    return { title: `${d} - 哈尔滨理工大学教务处`, link: l, language: `zh-CN`, item: p };
}
export { o as route };
