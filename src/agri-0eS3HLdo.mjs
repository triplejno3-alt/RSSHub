import"./ofetch-uhy-qh6X.mjs";import"./config-Cc-zZ5p-.mjs";import"./logger-_vmdpChp.mjs";import{t as e}from"./cache-DLkCV5c7.mjs";import"./helpers-C9wXLK0V.mjs";import{t}from"./parse-date-DjdQS_Nt.mjs";import{t as n}from"./got-CKQ7C9HX.mjs";import{t as r}from"./timezone-CrV-DT8S.mjs";import{Fragment as i,jsx as a,jsxs as o}from"hono/jsx/jsx-runtime";import{load as s}from"cheerio";import{renderToString as c}from"hono/jsx/dom/server";import{raw as l}from"hono/html";const u=({images:e,intro:t,description:n})=>c(o(i,{children:[e?.length?e.map(e=>e?.src?a(`figure`,{children:e.alt?a(`img`,{src:e.src,alt:e.alt}):a(`img`,{src:e.src})}):null):null,t?a(`blockquote`,{children:t}):null,n?l(n):null]})),d=async i=>{let{category:a=`zx/zxfb/`}=i.req.param(),o=i.req.query(`limit`)?Number.parseInt(i.req.query(`limit`),10):10,c=`http://www.agri.cn`,l=new URL(a.endsWith(`/`)?a:`${a}/`,c).href,{data:d}=await n(l),f=s(d),p=f(`html`).prop(`lang`),m=f(`div.list_li_con, div.nxw_video_com`).slice(0,o).toArray().map(e=>{e=f(e);let n=e.find(`a`).first(),r=n.text(),i=e.find(`img`).first().prop(`src`)?new URL(e.find(`img`).first().prop(`src`),c).href:void 0,a=u({intro:e.find(`p.con_text`).text()||void 0,images:i?[{src:i,alt:r}]:void 0});return{title:r,description:a,pubDate:t(e.find(`span.con_date_span`).text()||`${e.find(`div.com_time_p2`).text().trim()}${e.find(`div.com_time_p1`).text()}`,[`YYYY-MM-DD`,`YYYY.MM.DD`]),link:new URL(n.prop(`href`),l).href,content:{html:a,text:e.find(`p.con_text`).text()||void 0},image:i,banner:i,language:p}});m=await Promise.all(m.map(i=>e.tryGet(i.link,async()=>{let{data:e}=await n(i.link),a=s(e),o=a(`div.detailCon_info_tit`).text().trim(),c=u({description:a(`div.content_body_box`).html()||void 0});return i.title=o,i.description=c,i.pubDate=r(t(a(`meta[name="publishdate"]`).prop(`content`)),8),i.author=a(`meta[name="author"]`).prop(`content`)||a(`meta[name="source"]`).prop(`content`),i.content={html:c,text:a(`div.content_body_box`).text()},i.language=p,i.enclosure_url=a(`div.content_body_box video`).prop(`src`)??void 0,i.enclosure_type=i.enclosure_url?`video/mp4`:void 0,i.enclosure_title=i.enclosure_url?o:void 0,i})));let h=new URL(f(`div.logo img`).prop(`src`),c).href;return{title:f(`title`).text(),link:l,item:m,allowEmpty:!0,image:h,language:p}},f={path:`/:category{.+}?`,name:`分类`,url:`www.agri.cn`,maintainers:[`nczitzk`],handler:d,example:`/agri/zx/zxfb`,parameters:{category:"分类，默认为 `zx/zxfb`，即最新发布，可在对应分类页 URL 中找到"},description:`::: tip
