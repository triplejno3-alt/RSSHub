...
const i = {
    ...
    radar: [{ source: [`www.sankei.com/:category`], target: `/news/:category` }],
    name: `News`,
...
async function a(i) {
    let a = `https://www.sankei.com`,
        { category: o } = i.req.param(),
        s = `${a}/${o}/`,
        c = r((await n(s)).body),
        l = c(`header.story-card-header:first-of-type > h1`).text(),
        u = c(`div.story-card-feed.grid.hide_on_mobile > div > article`),
        d = await Promise.all(
            u.toArray().map((i) => {
                let o = c(i);
                o.find(`p a`).remove();
                let s = o.find(`div.story-card-flex > h2.headline > a`).text(),
                    l = a + (o.find(`div.story-card-flex > h2.headline > a`).attr(`href`) || ``),
