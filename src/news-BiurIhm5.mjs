...
import { t as a } from './description-B51rTNiE.mjs';
import { load as o } from 'cheerio';
const s = async (i) => {
        let { filter: s } = i.req.param(),
            c = Number.parseInt(i.req.query(`limit`) ?? `2`, 10),
            l = `https://kpopping.com`,
            u = new URL(`news${s ? `/${s}` : ``}`, l).href,
            d = o(await e(u)),
            f = d(`html`).attr(`lang`) ?? `en`,
            p = [];
        return (
            (p = d(`section.news-list-item`)
                .slice(0, c)
                .toArray()
                .map((e) => {
                    let t = d(e),
                        i = t.find(`h4.title-wr a`).last(),
                        a = i.text(),
                        o = t.find(`time.datetime-wr`).attr(`datetime`),
                        s = i.attr(`href`),
                        c = [t.find(`h4.title-wr a`).first()],
                        u = [...new Set(c.map((e) => d(e).text()).filter(Boolean))],
                        p = t
                            .find(`aside.author-wr`)
                            .toArray()
                            .map((e) => {
                                let n = d(e).find(`a`).last();
                                return { name: n.text(), url: new URL(n.attr(`href`), l).href, avatar: new URL(t.find(`aside.author-wr a img`).attr(`src`), l).href };
                            }),
                        m = o;
                    return {
                        title: a,
                        pubDate: o ? r(n(o, `MMM D, YYYY h:mma`), 8) : void 0,
                        link: s ? new URL(s, l).href : void 0,
                        category: u,
                        author: p,
                        doi: t.find(`meta[name="citation_doi"]`).attr(`content`),
                        updated: m ? r(n(m, `MMM D, YYYY h:mma`), 8) : void 0,
                        language: f,
                    };
                })),
            (p = (
                        ...
                        r.link
                            ? t.tryGet(r.link, async () => {
                                  let t = o(await e(r.link)),
                                      i = t(`h1`).contents().first().text(),
                                      s = a({ images: t(`figure.opening img`).attr(`src`) ? [{ src: new URL(t(`figure.opening img`).attr(`src`), l).href, alt: i }] : void 0, description: t(`div#article-content`).html() }),
                                      c = t(`meta[property="article:published_time"]`).attr(`content`),
                                      u = t(`aside.info a, div.supplements a.item`).toArray(),
                                      d = [...new Set(u.map((e) => t(e).text()?.trim()).filter(Boolean))],
                                      p = t(`div.content-snippet aside:not(.like)`)
                                          .toArray()
                                          .map((e) => {
                                              let n = t(e),
                                                  r = n.find(`a`).last();
                                              return { name: r.text(), url: new URL(r.attr(`href`), l).href, avatar: n.find(`img`).attr(`src`) };
                                          }),
                                      m = t(`meta[property="og:image"]`).attr(`content`),
                                      h = t(`meta[property="article:modified_time"]`).attr(`content`),
                                      g = { title: i, description: s, pubDate: c ? n(c) : r.pubDate, category: d, author: p, content: { html: s, text: s }, image: m, banner: m, updated: h ? n(h) : r.updated, language: f };
                                  return { ...r, ...g };
                              })
                            : r
                    )
