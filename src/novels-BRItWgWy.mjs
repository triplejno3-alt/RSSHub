...
[https://docs.rsshub.app/deploy/config#pixiv](https://docs.rsshub.app/deploy/config#pixiv)`}],requirePuppeteer:!1,antiCrawler:!1,supportBT:!1,supportPodcast:!1,supportScihub:!1,nsfw:!0},radar:[{title:`User Novels (簡介 Basic info)`,source:[`www.pixiv.net/users/:id/novels`,`www.pixiv.net/users/:id`,`www.pixiv.net/en/users/:id/novels`,`www.pixiv.net/en/users/:id`],target:`/user/novels/:id`},{title:`User Novels (全文 Full text)`,source:[`www.pixiv.net/users/:id/novels`,`www.pixiv.net/users/:id`,`www.pixiv.net/en/users/:id/novels`,`www.pixiv.net/en/users/:id`],target:`/user/novels/:id/true`}],name:`User Novels`,maintainers:[`TonyRL`,`SnowAgar25`],handler:C,description:`
...
- \`/pixiv/user/novels/79603797/true\` → 全文 Full text`},S=()=>!!(e.pixiv&&e.pixiv.refreshToken);async function C(e){let t=e.req.param(`id`),n=c(void 0,s(e.req.param(`full_content`)),!1),{limit:r}=e.req.query();if(S())return await v(t,n,r);let i=await b(t,n,r).catch(e=>{if(e.name===`Error`)return null;throw e});if(i)return i;throw new o(`This user may not have any novel works, or is an R18 creator, PIXIV_REFRESHTOKEN is required.
pixiv RSS is disabled due to the lack of relevant config.
