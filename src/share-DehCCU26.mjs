...
async function s(o) {
    let s = o.req.param(`category`) ?? `Recent`,
        c = `https://getquicker.net`,
        l = `${c}/Share/${s}`,
        u = a((await r({ method: `get`, url: l })).data),
        d = u(`table tbody tr`)
            .slice(1, o.req.query(`limit`) ? Number.parseInt(o.req.query(`limit`)) : 25)
            .toArray()
            .map((e) => ((e = u(e).find(`td a`).first()), { title: e.text(), link: `${c}${e.attr(`href`)}` }));
    return (
        (d = await Promise.all(
            d.map((o) =>
        ...
        { title: u(`title`).text(), link: l, item: d, allowEmpty: !0 }
    );
}
export { o as route };
