...
const i = {
    ...
    radar: [{ source: [`yicai.com/news/:id`, `yicai.com/news`], target: `/news/:id` }],
    name: `新闻`,
    maintainers: [`nczitzk`],
    handler: a,
    description: `| Id                       | 名称       |
...
async function a(i) {
    ...
    if (a) {
        ...
        for (let t of e.data.header.news)
            if (t.EnglishName === a || t.ChannelID === a) {
                o = { id: t.ChannelID, name: t.ChannelName, slug: t.EnglishName };
                break;
            }
    }
    let s = `${n}/news${a ? `/${o.slug}` : ``}`,
        c = await r(`${n}/api/ajax/${a ? `getlistbycid?cid=${o.id}` : `getjuhelist?action=news`}&page=1&pagesize=${i.req.query(`limit`) ?? 30}`, e.tryGet);
    return { title: `第一财经 - ${o?.name ?? `新闻`}`, link: s, item: c };
}
export { i as route };
