...
import { a as r, r as i } from './utils-BDg5Lhsa.mjs';
...
async function s(o) {
    let s = o.req.param(`category`) ?? ``,
        ...
        f = d(`.title`)
            .toArray()
            .map((e) => ((e = d(e)), { title: e.text(), link: `${l}${e.parent().attr(`href`)}`, guid: `https://18comic.org${e.parent().attr(`href`)}` }));
    return (
        (f = await Promise.all(
            f.map((r) =>
                e.tryGet(r.guid, async () => {
                    ...
                    return (
                        (r.pubDate = t(e(`.date`).first().text())),
                        e(`.d-flex`).remove(),
                        (r.author = e(`.blog_name_id`).first().text()),
                        (r.description = e(`.blog_content`).html()),
                        (r.category = e(`.panel-heading dropdown-toggle`)
                            .toArray()
                            .map((e) => d(e).text())),
                        r
                    );
        ...
        )),
        {
            title: d(`title`)
                .text()
                .replace(/最新的/, d(`.article-nav .active`).text()),
            link: u,
            item: f,
