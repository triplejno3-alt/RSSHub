...
async function a(i) {
    let a = `https://www.anthropic.com/news`,
        o = n(await e(a)),
        s = i.req.query(`limit`) ? Number.parseInt(i.req.query(`limit`), 10) : 20;
    return {
        ...
        item: await r(
            o(`.contentFadeUp a`)
                ...
                .map((e) => {
                    let t = o(e),
                        n = t.find(`h3`).text().trim(),
                        r = t.attr(`href`) ?? ``,
                        i = t.find(`p.detail-m.agate`).text().trim() || t.find(`div[class^="PostList_post-date__"]`).text().trim();
                    return { title: n, link: r.startsWith(`http`) ? r : `https://www.anthropic.com${r}`, pubDate: i };
                }),
            (r) =>
