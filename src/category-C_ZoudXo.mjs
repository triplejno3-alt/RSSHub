...
const n = {
    ...
    handler: r,
    radar: [{ title: `Category`, source: [`chikubi.jp/category/:keyword`], target: `/category/:keyword` }],
};
async function r(n) {
    let { keyword: r } = n.req.param(),
        { id: i, name: a } = await t(`category`, r),
        o = await e(`category`, i);
    return { title: `Category: ${a} - chikubi.jp`, link: `https://chikubi.jp/category/${r}`, item: o };
}
export { n as route };
