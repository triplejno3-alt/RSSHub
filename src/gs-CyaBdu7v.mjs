import"./ofetch-uhy-qh6X.mjs";import"./config-Cc-zZ5p-.mjs";import"./logger-_vmdpChp.mjs";import{t as e}from"./cache-DLkCV5c7.mjs";import"./helpers-C9wXLK0V.mjs";import{t}from"./parse-date-DjdQS_Nt.mjs";import{t as n}from"./got-CKQ7C9HX.mjs";import{load as r}from"cheerio";const i=async i=>{let{category:a=`xwdt`}=i.req.param(),o=i.req.query(`limit`)?Number.parseInt(i.req.query(`limit`),10):16,s=`https://gs.hust.edu.cn`,c=new URL(`${a}.htm`,s).href,{data:l}=await n(c),u=r(l),d=u(`div.btlist ul li`).slice(0,o).toArray().map(e=>{e=u(e);let n=e.find(`a`),r=n.prop(`href`);return{title:n.text(),pubDate:t(e.find(`span.time`).text()),link:r.startsWith(`http`)?r:new URL(r,s).href}});d=await Promise.all(d.map(t=>e.tryGet(t.link,async()=>{try{let{data:e}=await n(t.link),i=r(e),a=i(`div.v_news_content`).html();t.title=i(`div.article_title h1, h5`).text(),t.description=a,t.content={html:a,text:i(`div.v_news_content`).text()}}catch{}return t})));let f=u(`meta[name="keywords"]`).prop(`content`)?.replaceAll(`,`,` - `)??u(`title`).text(),p=new URL(u(`div.logo img`).prop(`src`),s).href;return{title:f,description:f.split(/-/).pop()?.trim(),link:c,item:d,allowEmpty:!0,image:p}},a={path:`/gs/:category{.+}?`,name:`研究生院`,url:`gs.hust.edu.cn`,maintainers:[`nczitzk`],handler:i,example:`/hust/gs/xwdt`,parameters:{category:"分类，默认为新闻动态，即 `xwdt`，可在对应分类页 URL 中找到"},description:`::: tip
