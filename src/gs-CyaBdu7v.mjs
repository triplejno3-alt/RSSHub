...
const i = async (i) => {
        let { category: a = `xwdt` } = i.req.param(),
            o = i.req.query(`limit`) ? Number.parseInt(i.req.query(`limit`), 10) : 16,
            s = `https://gs.hust.edu.cn`,
            c = new URL(`${a}.htm`, s).href,
            { data: l } = await n(c),
            u = r(l),
            d = u(`div.btlist ul li`)
                .slice(0, o)
                .toArray()
                .map((e) => {
                    e = u(e);
                    let n = e.find(`a`),
                        r = n.prop(`href`);
                    return { title: n.text(), pubDate: t(e.find(`span.time`).text()), link: r.startsWith(`http`) ? r : new URL(r, s).href };
                });
        d = await Promise.all(
            d.map((t) =>
                e.tryGet(t.link, async () => {
                    try {
                        let { data: e } = await n(t.link),
                            i = r(e),
                            a = i(`div.v_news_content`).html();
                        ((t.title = i(`div.article_title h1, h5`).text()), (t.description = a), (t.content = { html: a, text: i(`div.v_news_content`).text() }));
                    } catch {}
                    return t;
                })
            )
        );
        let f = u(`meta[name="keywords"]`).prop(`content`)?.replaceAll(`,`, ` - `) ?? u(`title`).text(),
            p = new URL(u(`div.logo img`).prop(`src`), s).href;
        return { title: f, description: f.split(/-/).pop()?.trim(), link: c, item: d, allowEmpty: !0, image: p };
    },
    a = {
        path: `/gs/:category{.+}?`,
        name: `研究生院`,
...
| [xwgz/xwdjs](https://rsshub.app/hust/gs/xwgz/xwdjs) | [xwgz/xwsy](https://rsshub.app/hust/gs/xwgz/xwsy) | [xwgz/dsdw](https://rsshub.app/hust/gs/xwgz/dsdw) |
  `,
        ...
        radar: [
            ...
            { title: `新闻动态`, source: [`gs.hust.edu.cn/xwdt.htm`], target: `/gs/xwdt` },
            { title: `研究生服务专区`, source: [`gs.hust.edu.cn/yjsfwzq.htm`], target: `/gs/yjsfwzq` },
            { title: `综合管理`, source: [`gs.hust.edu.cn/gzzd/zhgl.htm`], target: `/gs/gzzd/zhgl` },
            { title: `通知公告 - 课程及考试安排`, source: [`gs.hust.edu.cn/tzgg/kcjksap.htm`], target: `/gs/tzgg/kcjksap` },
            { title: `通知公告 - 国际交流`, source: [`gs.hust.edu.cn/tzgg/gjjl.htm`], target: `/gs/tzgg/gjjl` },
            { title: `通知公告 - 学位工作`, source: [`gs.hust.edu.cn/tzgg/xwgz.htm`], target: `/gs/tzgg/xwgz` },
            { title: `通知公告 - 同济医学院`, source: [`gs.hust.edu.cn/tzgg/tjyxy.htm`], target: `/gs/tzgg/tjyxy` },
            { title: `通知公告 - 其他`, source: [`gs.hust.edu.cn/tzgg/qt.htm`], target: `/gs/tzgg/qt` },
            { title: `学籍管理 - 学籍异动`, source: [`gs.hust.edu.cn/pygz/zbjs1/xjyd.htm`], target: `/gs/pygz/zbjs1/xjyd` },
            { title: `学籍管理 - 毕业管理`, source: [`gs.hust.edu.cn/pygz/zbjs1/bygl.htm`], target: `/gs/pygz/zbjs1/bygl` },
            { title: `教学管理 - 教学研究`, source: [`gs.hust.edu.cn/pygz/zbjs13/jxyj.htm`], target: `/gs/pygz/zbjs13/jxyj` },
            { title: `教学管理 - 课程教材`, source: [`gs.hust.edu.cn/pygz/zbjs13/kcjc.htm`], target: `/gs/pygz/zbjs13/kcjc` },
            { title: `教学管理 - 教学安排`, source: [`gs.hust.edu.cn/pygz/zbjs13/jxap.htm`], target: `/gs/pygz/zbjs13/jxap` },
            { title: `教学管理 - 课表查询`, source: [`gs.hust.edu.cn/pygz/zbjs13/kbcx.htm`], target: `/gs/pygz/zbjs13/kbcx` },
            { title: `培养过程 - 培养方案`, source: [`gs.hust.edu.cn/pygz/pygc/pyfa.htm`], target: `/gs/pygz/pygc/pyfa` },
            { title: `培养过程 - 硕博连读`, source: [`gs.hust.edu.cn/pygz/pygc/sbld.htm`], target: `/gs/pygz/pygc/sbld` },
            { title: `国际交流 - 国家公派项目`, source: [`gs.hust.edu.cn/pygz/zbjs11/gjgpxm.htm`], target: `/gs/pygz/zbjs11/gjgpxm` },
            { title: `国际交流 - 国际学术会议`, source: [`gs.hust.edu.cn/pygz/zbjs11/gjxshy.htm`], target: `/gs/pygz/zbjs11/gjxshy` },
            { title: `国际交流 - 校际合作项目`, source: [`gs.hust.edu.cn/pygz/zbjs11/xjhzxm.htm`], target: `/gs/pygz/zbjs11/xjhzxm` },
            { title: `国际交流 - 国际交流与合作办事流程`, source: [`gs.hust.edu.cn/pygz/zbjs11/gjjlyhzbslc.htm`], target: `/gs/pygz/zbjs11/gjjlyhzbslc` },
            { title: `专业学位 - 学位授权点目录`, source: [`gs.hust.edu.cn/pygz/zbjs111/xwsqdml.htm`], target: `/gs/pygz/zbjs111/xwsqdml` },
            { title: `专业学位 - 专业学位建设`, source: [`gs.hust.edu.cn/pygz/zbjs111/zyxwjs.htm`], target: `/gs/pygz/zbjs111/zyxwjs` },
            { title: `专业学位 - 特色培养`, source: [`gs.hust.edu.cn/pygz/zbjs111/tspy.htm`], target: `/gs/pygz/zbjs111/tspy` },
            { title: `学位工作 - 学位点建设`, source: [`gs.hust.edu.cn/xwgz/xwdjs.htm`], target: `/gs/xwgz/xwdjs` },
            { title: `学位工作 - 学位授予`, source: [`gs.hust.edu.cn/xwgz/xwsy.htm`], target: `/gs/xwgz/xwsy` },
            { title: `学位工作 - 导师队伍`, source: [`gs.hust.edu.cn/xwgz/dsdw.htm`], target: `/gs/xwgz/dsdw` },
        ],
    };
