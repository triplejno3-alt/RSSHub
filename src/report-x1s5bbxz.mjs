import"./ofetch-uhy-qh6X.mjs";import"./config-Cc-zZ5p-.mjs";import"./logger-_vmdpChp.mjs";import{t as e}from"./cache-DLkCV5c7.mjs";import"./helpers-C9wXLK0V.mjs";import{t}from"./parse-date-DjdQS_Nt.mjs";import{t as n}from"./got-CKQ7C9HX.mjs";import{Fragment as r,jsx as i,jsxs as a}from"hono/jsx/jsx-runtime";import{load as o}from"cheerio";import{renderToString as s}from"hono/jsx/dom/server";import{raw as c}from"hono/html";const l=({image:e,introduction:t,description:n})=>a(r,{children:[e?.src?i(`figure`,{children:i(`img`,{alt:e.alt,src:e.src})}):null,t?i(`blockquote`,{children:t}):null,n?c(n):null]}),u=e=>s(i(l,{...e})),d=(e,t=[])=>{for(let n of e){let{key:e,value:r,children:i}=n;t.push({key:e,value:r}),i&&i.length>0&&d(i,t)}return t},f={path:`/report/:industry?/:label?`,categories:[`new-media`],example:`/questmobile/report`,parameters:{industry:"行业，见下表，默认为 `-1`，即全部行业",label:"标签，见下表，默认为 `-1`，即全部标签"},features:{requireConfig:!1,requirePuppeteer:!1,antiCrawler:!1,supportBT:!1,supportPodcast:!1,supportScihub:!1},name:`行业研究报告`,maintainers:[`nczitzk`],handler:p,description:`::: tip
...
</details>`};async function p(r){let{industry:i,label:a}=r.req.param(),s=r.req.query(`limit`)?Number.parseInt(r.req.query(`limit`),10):50,c=`https://www.questmobile.com.cn`,l=new URL(`api/v2/report/article-list`,c).href,f=new URL(`api/v2/report/industry-label-tree`,c).href,{data:{data:{industryTree:p,labelTree:m}}}=await n(f),h=d(p),g=d(m),_=i?h.find(e=>e.key===i||e.value===i):void 0,v=a?g.find(e=>e.key===a||e.value===a):_?void 0:g.find(e=>e.key===i||e.value===i),y=_?.key??-1,b=v?.key??-1,x=new URL(`research/reports/${_?.key??-1}/${v?.key??-1}`,c).href,{data:S}=await n(l,{searchParams:{version:0,pageSize:s,pageNo:1,industryId:y,labelId:b}}),C=S.data.slice(0,s).map(e=>({title:e.title,link:new URL(`research/report/${e.id}`,c).href,description:u({image:{src:e.coverImgUrl,alt:e.title},introduction:e.introduction,description:e.content}),category:[...e.industryList??[],...e.labelList??[]],guid:`questmobile-report#${e.id}`,pubDate:t(e.publishTime)}));C=await Promise.all(C.map(r=>e.tryGet(r.link,async()=>{let{data:e}=await n(r.link),i=o(e);return i(`div.text div.daoyu`).remove(),r.title=i(`div.title h1`).text(),r.description+=u({description:i(`div.text`).html()}),r.author=i(`div.source`).text().replace(/^.*?：/,``),r.category=i(`div.hy, div.keyword`).find(`span`).toArray().map(e=>i(e).text()),r.pubDate=t(i(`div.data span`).prop(`datetime`)),r})));let{data:w}=await n(x),T=o(w),E=T(`meta[property="og:title"]`).prop(`content`).split(/-/)[0],D=[_?.value,v?.value].filter(Boolean),O=T(`img[alt="${E}"]`).prop(`src`),k=T(`link[rel="shortcut icon"]`).prop(`href`);return{item:C,title:`${E}${D.length===0?``:` - ${D.join(` - `)}`}`,link:x,description:T(`meta[property="og:description"]`).prop(`content`),language:`zh`,image:O,icon:k,logo:k,subtitle:T(`meta[name="keywords"]`).prop(`content`),author:E,allowEmpty:!0}}export{f as route};