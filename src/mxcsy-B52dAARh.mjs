...
const a = {
    ...
    example: `/ecnu/mxcsy`,
    radar: [{ source: [`mxcsy.ecnu.edu.cn`], target: `/mxcsy` }],
    name: `孟宪承书院通知公告`,
    maintainers: [`FrozenStarrrr`, `ChiyoYuki`, `ECNU-minus`],
    handler: async () => {
        let a = `https://mxcsy.ecnu.edu.cn/`,
            o = i((await n(`${a}29520/list.htm`)).data),
            s = o(`ul.wp_article_list > li`)
                .toArray()
                .map((e) => ({ pubDate: r(t(o(e).find(`.Article_PublishDate`).text()), 8), link: new URL(o(e).find(`a`).attr(`href`), a).toString(), title: o(e).find(`a`).text() }));
        return {
            title: `孟宪承书院通知公告`,
