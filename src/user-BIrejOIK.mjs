...
const u = {
    ...
    d = async (e, n) =>
        await t.tryGet(`behance:profile:${n}`, () => {
            let t = e.flatMap((e) => e.owners).find((e) => e.username === n);
            return Promise.resolve({ displayName: t.displayName, id: t.id, link: t.url, image: t.images.size_50.url.replace(`/user/50/`, `/user/source/`) });
        }),
    f = (e, t) =>
        c(
            o(i, {
                children: [
                    e?.length ? o(i, { children: [e, a(`br`, {})] }) : null,
                    t?.map((e) => {
                        if (e.__typename === `ImageModule`)
                            return o(i, {
                                children: [
                                    o(`figure`, { children: [a(`img`, { src: e.imageSizes?.allAvailable?.[0]?.url, alt: e.altText ?? void 0 }), e.caption?.length ? a(`figcaption`, { children: e.caption }) : null] }),
                                    a(`br`, {}),
                                ],
                            });
                        if (e.__typename === `TextModule`) return o(i, { children: [e.text ? l(e.text) : null, a(`br`, {})] });
                        if (e.__typename === `MediaCollectionModule`)
                            return o(i, { children: [e.components?.map((e) => a(`img`, { src: `https://mir-s3-cdn-cf.behance.net/project_modules/source/${e.filename}`, alt: `` })), a(`br`, {})] });
                        if (e.__typename === `EmbedModule`) {
                            let t = e.fluidEmbed || e.originalEmbed;
                            return o(i, { children: [t ? l(t) : null, a(`br`, {})] });
                        }
                        return o(i, { children: [`UNHANDLED MODULE: `, e.__typename, a(`br`, {})] });
                    }),
                ],
            })
        );
async function p(r) {
    let { user: i, type: a = `projects` } = r.req.param(),
        o = s.randomUUID(),
        c = { Cookie: `gk_suid=${Math.random().toString().slice(2, 10)}, gki=; originalReferrer=; bcp=${o}`, 'X-BCP': o, 'X-Requested-With': `XMLHttpRequest` },
        l = await e(`https://www.behance.net/v3/graphql`, {
            method: `POST`,
            headers: c,
            body: {
                query:
  ...
  fragment OwnerFields on User {
    ...
    creatorPro {
      ...
      initialSubscriptionDate
                ...
                variables: { username: i, after: `` },
            },
        }),
        u = a === `projects` ? l.data.user.profileProjects.nodes : l.data.user.appreciatedProjects.nodes,
        p = u.map((e) => ({
            title: e.name,
            link: e.url,
            author: e.owners.map((e) => e.displayName).join(`, `),
            image: e.covers.size_202.url.replace(`/202/`, `/source/`),
            pubDate: e.publishedOn ? n(e.publishedOn, `X`) : void 0,
            category: e.fields?.map((e) => e.label.toLowerCase()),
            projectId: e.id,
        })),
        m = await d(u, i),
        h = await Promise.all(
            p.map((r) =>
                t.tryGet(r.link, async () => {
                    let t = (
                        await e(`https://www.behance.net/v3/graphql`, {
                            method: `POST`,
                            headers: c,
                            body: {
                                query: `query ProjectPage($projectId: ProjectId!, $projectPassword: String) {
    viewer {
      ...Project_Viewer
    }
    ...
    images {
      allAvailable {
        ...
        type
                                ...
                                variables: { projectId: r.projectId },
                            },
                        })
                    ).data.project;
                    return (
                        (r.description = f(t.description, t.allModules)),
                        (r.category = [...new Set([...(r.category || []), ...(t.tags?.map((e) => e.title.toLowerCase()) || [])])]),
                        (r.pubDate = r.pubDate || (t.publishedOn ? n(t.publishedOn, `X`) : void 0)),
                        r
                    );
                })
            )
        );
    return { title: `${m.displayName}'s ${a}`, link: `https://www.behance.net/${i}/${a}`, image: m.image, item: h };
}
export { u as route };
