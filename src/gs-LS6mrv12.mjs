...
const i = {
    ...
    radar: [{ source: [`gs.tongji.edu.cn/tzgg.htm`, `gs.tongji.edu.cn/`] }],
    name: `研究生院通知公告`,
...
async function a(e) {
    return ((e.description = r((await n(e.link)).body)(`#vsb_content`).html()), e);
}
async function o() {
    let i = `https://gs.tongji.edu.cn`,
        o = r((await n(`${i}/tzgg.htm`)).body),
        s = o(`body > div > div.con_list.ma0a > div > div.list_content_right > div.list_list > ul`)
            .find(`li`)
            .toArray()
            .map((e) => {
                let n = o(e).find(`a`).attr(`title`),
                    r = o(e).find(`a`).attr(`href`);
                return { title: n, link: r.startsWith(`http`) ? r : `${i}/${r}`, pubDate: t(o(e).find(`span`).text(), `YYYY-MM-DD`) };
            });
    return {
        title: `同济大学研究生院`,
        link: i,
