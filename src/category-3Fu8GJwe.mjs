...
const o = {
    ...
    radar: [{ source: [`cosplaytele.com/category/:category`], target: `/category/:category` }],
    name: `Category`,
...
async function s(o) {
    let s = Number.parseInt(o.req.query(`limit`)) || 20,
        ...
        l = `${n}category/${c}/`,
        u = a((await t(l)).body),
        d = u(`#content .post-item`).slice(0, s).toArray();
    return {
        title: `${i} - Category: ${c}`,
        link: l,
        item: await Promise.all(
            d.map((t) => {
                let n = u(t).find(`h5.post-title a`).attr(`href`);
