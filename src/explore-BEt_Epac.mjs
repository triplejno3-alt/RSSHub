...
const a = async (t) => {
        let { filter: a } = t.req.param(),
            o = Number.parseInt(t.req.query(`limit`) ?? `30`, 10),
            s = new URL(`/${a ? `home/` : ``}explore/${a ?? ``}`, n).href,
            c = i(await e(s)),
        ...
        return {
            title: `${c(`title`).text()} - ${c(`li.active`)
                .slice(1)
                .toArray()
                .map((e) => c(e).text())
                .join(`|`)}`,
    ...
    o = {
        ...
        parameters: { category: `过滤器，默认为空，可在对应页 URL 中找到` },
        description:
