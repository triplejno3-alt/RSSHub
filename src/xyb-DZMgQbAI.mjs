...
const a = `https://xyb.qd.sdu.edu.cn/`,
    o = { gztz: { title: `工作通知`, url: `gztz.htm` } },
    s = {
        path: `/qd/xyb/:type?`,
    ...
    let c = s.req.param(`type`) ?? `gztz`,
        ...
        d = u(`.list li`)
            ...
            .map((e) => {
                e = u(e);
                let n = e.find(`a`);
                return { title: n.text().slice(1).trim(), link: n.attr(`href`).startsWith(`info/`) ? a + n.attr(`href`) : n.attr(`href`), pubDate: t(e.find(`b`).text().trim(), `YYYY-MM-DD`) };
            });
    return (
        ...
        { title: `山东大学（青岛）学研办${o[c].title}`, description: u(`title`).text(), link: l, item: d }
    );
}
export { s as route };
