...
async function s(o) {
    let s = o.req.param(`type`) ?? `xstz`,
        c = a.get(s);
    if (!c) throw new n(`invalid type`);
    let l = `https://jwc.njust.edu.cn` + c.id + `/list.htm`,
        u = i(await r(l, !0)),
        d = u(`div#wp_news_w3`).find(`tr`);
    return { title: c.title, link: l, item: d.toArray().map((n) => ({ title: u(n).find(`a`).attr(`title`).trim(), pubDate: t(e(u(n).find(`td[width="14%"]`).text(), `YYYY-MM-DD`), 8), link: u(n).find(`a`).attr(`href`) })) };
}
export { o as route };
