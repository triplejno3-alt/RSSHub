...
const l = {
    ...
    radar: [{ source: [`thegadgetflow.com/categories/:category`], target: `/:category` }],
    name: `Category`,
    maintainers: [`EthanWng97`],
    handler: u,
};
async function u(l) {
    let u = `https://thegadgetflow.com`,
    ...
    let m = p?.id,
        h = p?.name,
        g = p?.link,
        _ = (await n(`${u}/wp-json/wp/v2/posts`, { searchParams: { per_page: l.req.query(`limit`) ? Number.parseInt(l.req.query(`limit`)) : 10, _embed: ``, ...(m && { categories: m }) } })).data.map((e) => {
            ...
            return (
                n?.caption?.rendered && (d = o(n?.caption?.rendered)),
                {
                    ...
                    description: s(a(r, { children: [l ? a(`figure`, { children: [i(`img`, { src: l, alt: u }), i(`figcaption`, { children: d?.text() || `` })] }) : null, e.content.rendered ? c(e.content.rendered) : null] })),
                    link: e.link,
    ...
    return { title: h ? `Gadget Flow - ${h}` : `Gadget Flow`, link: g || `${u}/${f}`, item: _ };
}
export { l as route };
