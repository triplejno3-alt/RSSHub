import"./ofetch-uhy-qh6X.mjs";import"./config-Cc-zZ5p-.mjs";import"./logger-_vmdpChp.mjs";import"./cache-DLkCV5c7.mjs";import"./helpers-C9wXLK0V.mjs";import{t as e}from"./parse-date-DjdQS_Nt.mjs";import"./got-CKQ7C9HX.mjs";import"./timezone-CrV-DT8S.mjs";import{t}from"./rss-parser-CKuAfhVS.mjs";import{a as n,i as r,o as i,r as a,s as o,t as s}from"./util-Dq2Dir9E.mjs";const c={path:`/cid/:id?`,name:`更多分类`,parameters:{id:`分类，见下表，留空为直播`},example:`/mydrivers/cid/2`,maintainers:[`nczitzk`],handler:l,url:`m.mydrivers.com/`,radar:[{source:[`m.mydrivers.com/`],target:`/zhibo`}],description:`::: details 更多分类
...
:::`};async function l(c){let l=c.req.param(`id`),u=c.req.query(`limit`)?Number.parseInt(c.req.query(`limit`),10):100,d=l?`?cid=${l}`:``,f=new URL(l?`newsclass.aspx${d}`:``,i).href,p=new URL(`rss.aspx${d}`,n).href,m=await t.parseURL(p),h=m.items.slice(0,u).map(t=>({title:t.title,link:t.link,description:t.content,author:t.creator,category:t.categories,guid:t.guid.match(/\/(\d+)\.htm/)[1],pubDate:e(t.isoDate)}));return l&&(h=await r(h)),{...await a(f),item:h,title:`${o} - ${m.title.split(/_/).pop()||s.zhibo}`}}export{c as route};