...
import { a as n, i as r, o as i, r as a, s as o, t as s } from './util-Dq2Dir9E.mjs';
const c = {
    path: `/cid/:id?`,
    name: `更多分类`,
...
async function l(c) {
    let l = c.req.param(`id`),
        u = c.req.query(`limit`) ? Number.parseInt(c.req.query(`limit`), 10) : 100,
        d = l ? `?cid=${l}` : ``,
        f = new URL(l ? `newsclass.aspx${d}` : ``, i).href,
        p = new URL(`rss.aspx${d}`, n).href,
        m = await t.parseURL(p),
        h = m.items.slice(0, u).map((t) => ({ title: t.title, link: t.link, description: t.content, author: t.creator, category: t.categories, guid: t.guid.match(/\/(\d+)\.htm/)[1], pubDate: e(t.isoDate) }));
    return (l && (h = await r(h)), { ...(await a(f)), item: h, title: `${o} - ${m.title.split(/_/).pop() || s.zhibo}` });
}
export { c as route };
