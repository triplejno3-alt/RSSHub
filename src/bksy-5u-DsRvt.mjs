...
const a = {
    ...
    example: `/ecnu/bksy`,
    radar: [{ source: [`bksy.ecnu.edu.cn`], target: `/bksy` }],
    name: `本科生院通知`,
    maintainers: [`FrozenStarrrr`, `ChiyoYuki`, `ECNU-minus`],
    handler: async () => {
        let a = `https://bksy.ecnu.edu.cn/`,
            o = i((await n(`${a}tzggwwxsgg/list.htm`)).data),
            s = o(`ul.news_list.list2 > li`)
                .toArray()
                .map((e) => ({ pubDate: r(t(o(e).find(`.news_date`).text()), 8), link: new URL(o(e).find(`a`).attr(`href`), a).toString(), title: o(e).find(`.news_title`).text() }));
        return {
            title: `本科生院通知`,
