...
const s = {
    ...
    handler: c,
    radar: [{ source: [`cara.app/:user`, `cara.app/:user/*`], target: `/timeline/:user` }],
};
async function c(s) {
    let c = s.req.param(`user`),
        l = s.req.query(`limit`) ? Number.parseInt(s.req.query(`limit`), 10) : 15,
        u = await i(c),
        d = (await a(`${n}/posts/getAllByUser?slug=${u.slug}&take=${l}`)).data.map((n) => {
    ...
    return { title: `Timeline - ${u.name}`, link: `${r}/${c}/all`, image: `${t}/${u.photo}`, item: d };
}
export { s as route };
