...
const i = { ggl: { title: `通知公告`, description: `CATTI 考试通知和公告` }, ywdt: { title: `要闻动态`, description: `CATTI 考试要闻动态` }, zxzc: { title: `最新政策`, description: `CATTI 考试最新政策` } },
    a = {
        path: `/news/:category`,
        name: `CATTI 考试消息`,
...
`,
        handler: async (a) => {
            let o = a.req.param(`category`),
                s = `https://www.catticenter.com/${o}`,
                { data: c } = await n(s),
                l = r(c),
                u = l(`ul.ui-card.ui-card-a > li`)
                    .toArray()
                    .map((e) => {
                        let n = l(e).find(`span.ui-right-time`).text(),
                            r = l(e).find(`a`).attr(`title`),
                            i = `https://www.catticenter.com${l(e).find(`a`).attr(`href`)}`;
                    ...
                    u.map((t) =>
                        e.tryGet(t.link, async () => {
                            let { data: e } = await n(t.link),
                                i = r(e)(`div.ui-article-cont`).html() || ``;
                            return {
                                title: t.title,
                                pubDate: t.date,
                                link: t.link,
        ...
        categories: [`study`],
        parameters: { category: `消息分类名，可在下面的描述中找到。` },
        ...
        radar: [{ source: [`www.catticenter.com/:category`] }],
    };
export { a as route };
