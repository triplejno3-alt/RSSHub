...
const u = ({ intro: e, description: t }) => c(o(i, { children: [e ? a(`blockquote`, { children: e }) : null, t ? l(t) : null] })),
    d = async (r) => {
        let { category: i = `zxyw` } = r.req.param(),
            a = Number.parseInt(r.req.query(`limit`) ?? `11`, 10),
            o = new URL(i.endsWith(`/`) ? i : `${i}/`, `https://www.chinacdc.cn`).href,
            c = s(await e(o)),
            l = c(`html`).prop(`lang`),
            d = c(`ul.xw_list li`)
                .slice(0, a)
                ...
                .map((e) => {
                    let t = c(e),
                        r = t.find(`a`),
                        i = r.prop(`title`) === void 0 ? r.text() : r.prop(`title`),
                        a = i.match(/(\d{4}-\d{2}-\d{2})$/),
                        s,
                        d = i;
                    if (a) ((s = n(a[0])), (d = i.replace(/\d{4}-\d{2}-\d{2}$/, ``).trim()));
                    else {
                        let e = t.find(`span`).text().trim();
                        s = e ? n(e) : void 0;
                    }
                    let f = u({ intro: t.find(`p.zy`).text() }),
                        p = t.find(`img`).prop(`src`),
                        m = p?.split(/\./).pop(),
                        h = p ? new URL(p, o).href : void 0,
                        g = {};
                    return (
                        m && h && (g[m] = { url: h }),
                        {
                            ...
                            description: f,
                            pubDate: s,
                            link: new URL(r.prop(`href`), o).href,
                            content: { html: f, text: t.find(`p.zy`).text() },
                            image: h,
                            banner: h,
                            language: l,
                            media: Object.keys(g).length > 0 ? g : void 0,
                        }
                    );
                });
        d = (
                    ...
                    !r.link && typeof r.link != `string`
                        ...
                        : t.tryGet(r.link, async () => {
                              let t = s(await e(r.link)),
                                  i = t(`h5`).text(),
                                  a = u({ description: t(`div.TRS_Editor`).html() || void 0 }),
                                  o = t(`span.fb em`).text().trim(),
                                  c = o ? n(o) : r.pubDate;
                              return { title: i || r.title, description: a, link: r.link, pubDate: c, content: { html: a, text: t(`div.TRS_Editor`).text() }, image: r.image, banner: r.banner, language: l, media: r.media };
                          })
                )
            )
        ).filter((e) => !0);
        let f = c(`title`).text(),
            p = c(`div.erjiCurNav`).text(),
            m = new URL(c(`img.logo`).prop(`src`), o).href;
        return { title: `${f} - ${p}`, description: p, link: o, item: d, allowEmpty: !0, image: m, author: f, language: l, id: o };
    },
    f = {
        path: `/:category{.+}?`,
        name: `通用`,
        ...
        parameters: { category: '分类，默认为 `zxyw`，即中心要闻，可在对应分类页 URL 中找到，Category, `zxyw`，即中心要闻 by default' },
        description: `::: tip
...
`,
        ...
        radar: [
            ...
            { title: `中心要闻`, source: [`www.chinacdc.cn/zxyw/`], target: `/zxyw` },
            { title: `通知公告`, source: [`www.chinacdc.cn/tzgg/`], target: `/tzgg` },
            { title: `党建园地 - 廉政文化`, source: [`www.chinacdc.cn/djgz_13611/`], target: `/dqgz/djgz_13611` },
            { title: `党建园地 - 党建工作`, source: [`www.chinacdc.cn/dqgz/`], target: `/dqgz/djgz` },
            { title: `党建园地 - 廉政文化`, source: [`www.chinacdc.cn/djgz_13611/`], target: `/dqgz/djgz_13611` },
            { title: `党建园地 - 工会工作`, source: [`www.chinacdc.cn/ghgz/`], target: `/dqgz/ghgz` },
            { title: `党建园地 - 团青工作`, source: [`www.chinacdc.cn/tqgz/`], target: `/dqgz/tqgz` },
            { title: `党建园地 - 理论学习`, source: [`www.chinacdc.cn/tqgz_13618/`], target: `/dqgz/tqgz_13618` },
            { title: `疾控应急 - 传染病`, source: [`www.chinacdc.cn/jkyj/crb2/`], target: `/jkyj/crb2` },
            { title: `疾控应急 - 突发公共卫生事件`, source: [`www.chinacdc.cn/jkyj/tfggws/`], target: `/jkyj/tfggws` },
            { title: `疾控应急 - 慢性病与伤害防控`, source: [`www.chinacdc.cn/jkyj/mxfcrxjb2/`], target: `/jkyj/mxfcrxjb2` },
            { title: `疾控应急 - 烟草控制`, source: [`www.chinacdc.cn/jkyj/yckz/`], target: `/jkyj/yckz` },
            { title: `疾控应急 - 营养与健康`, source: [`www.chinacdc.cn/jkyj/yyyjk2/`], target: `/jkyj/yyyjk2` },
            { title: `疾控应急 - 环境与健康`, source: [`www.chinacdc.cn/jkyj/hjyjk/`], target: `/jkyj/hjyjk` },
            { title: `疾控应急 - 职业卫生与中毒控制`, source: [`www.chinacdc.cn/jkyj/hjwsyzdkz/`], target: `/jkyj/hjwsyzdkz` },
            { title: `疾控应急 - 放射卫生`, source: [`www.chinacdc.cn/jkyj/fsws/`], target: `/jkyj/fsws` },
            { title: `疾控应急 - 免疫规划`, source: [`www.chinacdc.cn/jkyj/mygh02/`], target: `/jkyj/mygh02` },
            { title: `疾控应急 - 结核病防控`, source: [`www.chinacdc.cn/jkyj/jhbfk/`], target: `/jkyj/jhbfk` },
            { title: `疾控应急 - 寄生虫病`, source: [`www.chinacdc.cn/jkyj/jscb/`], target: `/jkyj/jscb` },
            { title: `科学研究 - 科技进展`, source: [`www.chinacdc.cn/kxyj/kjjz/`], target: `/kxyj/kjjz` },
            { title: `科学研究 - 学术动态`, source: [`www.chinacdc.cn/kxyj/xsdt/`], target: `/kxyj/xsdt` },
            { title: `科学研究 - 科研平台`, source: [`www.chinacdc.cn/kxyj/xsjl/`], target: `/kxyj/xsjl` },
            { title: `科学研究 - 科研亮点`, source: [`www.chinacdc.cn/kxyj/kyld/`], target: `/kxyj/kyld` },
            { title: `科学研究 - 科技政策`, source: [`www.chinacdc.cn/kxyj/kjzc/`], target: `/kxyj/kjzc` },
            { title: `教育培训 - 研究生院`, source: [`www.chinacdc.cn/jypx/yjsy/`], target: `/jypx/yjsy` },
            { title: `教育培训 - 继续教育`, source: [`www.chinacdc.cn/jypx/jxjy/`], target: `/jypx/jxjy` },
            { title: `教育培训 - 博士后`, source: [`www.chinacdc.cn/jypx/bsh/`], target: `/jypx/bsh` },
            { title: `教育培训 - 中国现场流行病学培训项目（CFETP）`, source: [`www.chinacdc.cn/jypx/CFETP/`], target: `/jypx/CFETP` },
            { title: `全球公卫 - 合作伙伴`, source: [`www.chinacdc.cn/qqgw/hzhb/`], target: `/qqgw/hzhb` },
            { title: `全球公卫 - 世界卫生组织合作中心和参比实验室`, source: [`www.chinacdc.cn/qqgw/wszz/`], target: `/qqgw/wszz` },
            { title: `全球公卫 - 国际交流 (港澳台交流)`, source: [`www.chinacdc.cn/qqgw/gjjl/`], target: `/qqgw/gjjl` },
            { title: `全球公卫 - 公共卫生援外与合作`, source: [`www.chinacdc.cn/qqgw/ggws/`], target: `/qqgw/ggws` },
            { title: `人才建设 - 院士风采`, source: [`www.chinacdc.cn/rcjs/ysfc/`], target: `/rcjs/ysfc` },
            { title: `人才建设 - 首席专家`, source: [`www.chinacdc.cn/rcjs/sxzj/`], target: `/rcjs/sxzj` },
            { title: `人才建设 - 人才队伍`, source: [`www.chinacdc.cn/rcjs/rcdw/`], target: `/rcjs/rcdw` },
            { title: `人才建设 - 人才招聘`, source: [`www.chinacdc.cn/rcjs/rczp/`], target: `/rcjs/rczp` },
            { title: `健康数据 - 全国法定传染病疫情情况`, source: [`www.chinacdc.cn/jksj/jksj01/`], target: `/jksj/jksj01` },
            { title: `健康数据 - 全国新型冠状病毒感染疫情情况`, source: [`www.chinacdc.cn/jksj/xgbdyq/`], target: `/jksj/xgbdyq` },
            { title: `健康数据 - 重点传染病和突发公共卫生事件风险评估报告`, source: [`www.chinacdc.cn/jksj/jksj02/`], target: `/jksj/jksj02` },
            { title: `健康数据 - 全球传染病事件风险评估报告`, source: [`www.chinacdc.cn/jksj/jksj03/`], target: `/jksj/jksj03` },
            { title: `健康数据 - 全国预防接种异常反应监测信息概况`, source: [`www.chinacdc.cn/jksj/jksj04_14209/`], target: `/jksj/jksj04_14209` },
            { title: `健康数据 - 流感监测周报`, source: [`www.chinacdc.cn/jksj/jksj04_14249/`], target: `/jksj/jksj04_14249` },
            { title: `健康数据 - 全国急性呼吸道传染病哨点监测情况`, source: [`www.chinacdc.cn/jksj/jksj04_14275/`], target: `/jksj/jksj04_14275` },
            { title: `健康数据 - 健康报告`, source: [`www.chinacdc.cn/jksj/jksj04/`], target: `/jksj/jksj04` },
            { title: `健康科普 - 传染病`, source: [`www.chinacdc.cn/jkkp/crb/`], target: `/jkkp/crb` },
            { title: `健康科普 - 慢性非传染性疾病`, source: [`www.chinacdc.cn/jkkp/mxfcrb/`], target: `/jkkp/mxfcrb` },
            { title: `健康科普 - 免疫规划`, source: [`www.chinacdc.cn/jkkp/mygh/`], target: `/jkkp/mygh` },
            { title: `健康科普 - 公共卫生事件`, source: [`www.chinacdc.cn/jkkp/ggws/`], target: `/jkkp/ggws` },
            { title: `健康科普 - 烟草控制`, source: [`www.chinacdc.cn/jkkp/yckz/`], target: `/jkkp/yckz` },
            { title: `健康科普 - 营养与健康`, source: [`www.chinacdc.cn/jkkp/yyjk/`], target: `/jkkp/yyjk` },
            { title: `健康科普 - 环境健康`, source: [`www.chinacdc.cn/jkkp/hjjk/`], target: `/jkkp/hjjk` },
            { title: `健康科普 - 职业健康与中毒控制`, source: [`www.chinacdc.cn/jkkp/zyjk/`], target: `/jkkp/zyjk` },
            { title: `健康科普 - 放射卫生`, source: [`www.chinacdc.cn/jkkp/fsws/`], target: `/jkkp/fsws` },
        ],
        view: r.Articles,
    };
