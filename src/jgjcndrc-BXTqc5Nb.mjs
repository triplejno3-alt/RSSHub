...
const a = async (a) => {
        let { columnId: o = `1832739866673426433`, subColumnId: s } = a.req.param(),
            c = a.req.query(`limit`) ? Number.parseInt(a.req.query(`limit`), 10) : 30,
            l = `https://www.jgjcndrc.org.cn`,
            u = new URL(`list?clmId=${o}${s ? `&sclmId=${s}` : ``}`, l).href,
            d = new URL(`prod-api/portal/artPageByColumn/${s ?? o}`, l).href,
            { data: f } = await n(u),
            p = i(f),
            m = p(`html`).prop(`lang`),
            { data: h } = await n(d, { searchParams: { pageNum: 1, pageSize: c } }),
            g = h.data.slice(0, c).map((e) => {
                let n = `jgjcndrc-${e.articleId}`;
                return { title: e.articleTitle, pubDate: t(e.pubDate), link: new URL(`prod-api/portal/article/${e.articleId}`, l).href, guid: n, id: n, language: m };
            });
        g = await Promise.all(
            g.map((i) =>
                e.tryGet(i.link, async () => {
                    let { data: e } = await n(i.link),
                        ...
                        o = a.articleTitle,
                        s = a.articleContent,
                        c = `jgjcndrc-${a.articleId}`;
                    return (
                        (i.title = o),
                        (i.description = s),
                        (i.pubDate = r(t(a.pubDate), 8)),
                        (i.link = a.linkUrl || new URL(a.articleUrl, l).href),
                        (i.category = a.parentColumns.map((e) => e.columnName)),
                        (i.guid = c),
                        (i.id = c),
                        (i.content = { html: s, text: s }),
                        (i.language = m),
                        i
                    );
                })
        ...
        let _ = new URL(p(`header img`).last().prop(`src`), l).href;
        return {
            title: `${p(`title`).text()}${p(`div.tit`).text() ? ` - ${p(`div.tit`).text()}` : ``}`,
            description: p(`meta[name="description"]`).prop(`content`),
            link: u,
            item: g,
            allowEmpty: !0,
            image: _,
            author: p(`header h1`).text(),
            language: m,
        };
    },
    o = {
        ...
        parameters: { columnId: '栏目 id，默认为 `1832739866673426433`，即通知公告，可在对应栏目页 URL 中找到', subColumnId: `子栏目 id，默认为空，可在对应子栏目页 URL 中找到` },
        description:
        ...
        radar: [
            {
                ...
                target: (e, t) => {
                    t = new URL(t);
                    let n = t.searchParams.get(`clmId`),
                        r = t.searchParams.get(`sclmId`);
                    return `/jgjcndrc${n ? `/${n}${r ? `/${r}` : ``}` : ``}`;
                },
            },
        ],
    };
