...
async function s(o) {
    let s = `https://www.swpu.edu.cn/bgw/${o.req.param(`code`)}.htm`,
        c = a((await n.get(s)).data),
        l = c(`.title`).text(),
        u = c(`.notice > ul > li > a`)
            .toArray()
            .map((e) => ({ title: c(e.children[0]).text(), pubDate: r(t(c(e.children[1]).text()), 8), link: i(`https://www.swpu.edu.cn`, c(e).attr(`href`)) })),
        d = await Promise.all(
            u.map(
                async (t) =>
                    await e.tryGet(t.link, async () => {
                        ...
                        if (e(`title`).text().startsWith(`系统提示`)) ((t.author = `系统`), (t.description = `无权访问`));
                            ...
                            ((t.author = `办公网`), (t.description = e(`.v_news_content`).html()));
    ...
    return { title: `西南石油大学办公网 ${l}`, link: s, description: `西南石油大学办公网 ${l} 列表`, language: `zh-CN`, item: d };
}
export { o as route };
