...
async function a(i) {
    let { id: a, order: o = `new` } = i.req.param(),
        s = new URL(`${n}/trading/listing.php`, n);
    (s.searchParams.append(`id`, a), s.searchParams.append(`order`, o), s.searchParams.append(`type`, `all`));
    let c = r(await e(s.href)),
        l = c(`.item_grid_wrap div a`)
            .toArray()
            .map((e) => ((e = c(e)), { title: e.find(`.lazyloadx`).attr(`alt`), link: new URL(e.attr(`href`), s.href).href, author: e.find(`.trade_info div span`).eq(1).text() })),
