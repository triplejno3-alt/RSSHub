...
async function o(a) {
    let o = a.req.param(`category`) ?? ``,
        ...
        l = Object.keys(c).includes(o) ? `${s}/${c[o].tag}/list.htm` : `${s}/39283/list.htm`,
        u = i((await n(l)).data),
        d = u(`div.con1rm2rt.xi20`)
            .toArray()
            .map((e) => {
                let t = u(e).find(`a`),
                    n = t.text(),
                    r = t.attr(`href`);
                return !r || /https?:\/\//i.test(r) ? null : { title: n, link: new URL(r, s).href };
            })
            .filter((e) => e !== null),
        f = await Promise.all(
            d.map((a) =>
                e.tryGet(a.link, async () => {
                    let e = i((await n(a.link)).data),
                        ...
                        s = r(t(o ? o[0] : ``), 8),
                        c = e(`div.wp_articlecontent`).html();
                    return { title: a.title, pubDate: s, link: a.link, guid: a.link, description: c };
                })
            )
        );
    return { title: `浙江大学控制学院通知 - ${c[o]?.title || `简讯专栏`}`, link: l, item: f };
}
export { a as route };
