...
import { n as t, r as n, t as r } from './utils-ls53K0so.mjs';
const i = {
    path: `/latest/:type?`,
    ...
    handler: async (i) => {
        let { type: a = `latest` } = i.req.param(),
            o = i.req.query(`limit`) ? Number.parseInt(i.req.query(`limit`), 10) : 20,
            s = { latest: { title: `最新`, apiType: `newest` }, heat: { title: `熱議`, apiType: `hottest` }, essence: { title: `精華`, apiType: `icymi` } },
            c = (
                await e(t, {
                    method: `POST`,
                    body: {
            ...
            ).data.viewer.recommendation.feed.edges.map(({ node: e }) => n(e));
        return { title: `Matters | ${s[a].title}`, link: r, item: c };
    },
