...
async function o(a) {
    let o = a.req.param(`tag`) ?? `過去24小時`,
        s = `https://global.udn.com`,
        c = `${s}/search/tagging/1020/${o}`,
        l = (await n({ method: `get`, url: `${s}/search/ajax_tag/1020/${o}` })).data.articles.map((e) => ({ title: e.art_title, link: `${s}${e.link}` }));
    return (
        (l = await Promise.all(
            l.map((a) =>
        ...
        { title: `轉角國際 udn Global - ${o}`, link: c, item: l }
    );
}
export { a as route };
