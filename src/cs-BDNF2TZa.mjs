...
async function s(o) {
    let s = o.req.param(`type`) ?? `xyxw`,
        c = a.get(s);
    if (!c) throw new n(`invalid type`);
    let l = `https://cs.njust.edu.cn` + c.id + `/list.htm`,
        u = i(await r(l, !0)),
        d = u(`div#wp_news_w9`).find(`a`);
    return {
        title: c.title,
        link: l,
        item: d
            .toArray()
            .map((n) => ({ title: u(n).find(`span[class="column-news-title"]`).text().trim(), pubDate: t(e(u(n).find(`span[class="column-news-date news-date-hide"]`).text(), `YYYY-MM-DD`), 8), link: u(n).attr(`href`) })),
    };
}
export { o as route };
