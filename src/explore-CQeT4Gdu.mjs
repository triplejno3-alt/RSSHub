...
async function i(r) {
    let { type: i = `rec`, category: a = `所有` } = r.req.param(),
        o = `https://afdian.com`,
        s = (await e(`${o}/api/creator/list`, { searchParams: { type: i, category_id: t[a] } })).data.data.list.map((e) => {
            let { doing: t, monthly_fans: n, detail: r } = e.creator;
            return {
                title: e.name,
                description: `正在创作 ${t}<br/>
            ${n}发电人次/月<br/>
            详情:<br/>
            ${r}<br/>
            `,
    ...
    return { title: `爱发电-创作者 (按 ${a}/${n[i]})`, description: `爱发电-发现创作者 (按 ${a}/${n[i]})`, link: `${o}/explore`, item: s };
}
export { r as route };
