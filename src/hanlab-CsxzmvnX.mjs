...
async function i() {
    let r = `https://hanlab.mit.edu`,
        i = `${r}/blog`,
        a = n((await t(i)).data);
    return {
        title: `MIT HAN Lab Blog`,
        link: i,
        item: a(`a.post-card-wrapper`)
            .toArray()
            .map((t) => {
                let n = a(t);
                return { title: n.find(`h3.text-title`).text().trim(), link: new URL(n.attr(`href`) ?? ``, r).href, description: n.find(`p.text-tldr`).html() ?? void 0, pubDate: e(n.find(`div.text-date`).text().trim()) };
            }),
    };
}
export { r as route };
