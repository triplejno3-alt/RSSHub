...
const i = {
    ...
    radar: [{ source: [`whitehouse.gov/:category`, `whitehouse.gov/`], target: `/news/:category` }],
    name: `News`,
...
async function a(i) {
    let a = `https://www.whitehouse.gov/${i.req.param(`category`) ?? `news`}/`,
        ...
        s = o(`.post`)
            ...
            .map((e) => {
                ...
                return { title: n.text(), link: n.attr(`href`), pubDate: t(e.find(`time`).attr(`datetime`)), category: [e.find(`a[rel^=tag]`).first().text()] };
            }),
        c = await Promise.all(
            s.map((t) =>
                e.tryGet(t.link, async () => {
                    let e = r((await n({ method: `get`, url: t.link })).data);
                    return (e(`.wp-block-whitehouse-topper`).remove(), (t.description = e(`.entry-content`).html()), t);
                })
            )
        );
