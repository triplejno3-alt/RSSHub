...
const n = {
    ...
    radar: [{ source: [`link3.to/events`], target: `/events` }],
    handler: r,
};
async function r() {
    return {
        ...
        author: `Link3`,
        item: (
            await e(`https://api.cyberconnect.dev/profile/`, {
                method: `POST`,
        ...
        ).data.trendingEvents.list.map((e) => ({
            title: e.title,
            link: `https://link3.to/e/${e.id}`,
            description: e.info ?? ``,
            author: e.organizer.lightInfo.displayName,
            guid: e.id,
            pubDate: t(e.startTimestamp * 1e3),
