...
async function o(n) {
    let a = n.req.param(`site`),
        o = n.req.param(`only_media`) === `true` ? `true` : `false`;
    ...
    let s = `http://${a}/api/v1/timelines/public?remote=true&only_media=${o}`,
        c = (await t.get(s, { headers: i.apiHeaders(a) })).data;
    return { title: `Federated Public${n.req.param(`only_media`) === `true` ? ` Media` : ``} Timeline on ${a}`, link: `https://${a}`, item: i.parseStatuses(c) };
}
export { a as route };
