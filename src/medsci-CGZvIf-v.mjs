...
async function o(a) {
    let o = a.req.param(`sid`) ?? ``,
        s = a.req.param(`tid`) ?? ``;
    o = o === `recommend` ? `` : o;
    let c = `https://www.medsci.cn`,
        l = `${c}${o ? `/department/details?s_id=${o}&module=article${s ? `&t_id=${s}` : ``}` : ``}`,
        ...
        d = u(`#articleList`)
            ...
            .map((e) => {
                e = u(e);
                let r = e.parent().parent().find(`.item-meta-item`).first().text();
                return { title: e.text(), link: `${c}${e.attr(`href`).replace(/;jsessionid=[\dA-Z]+/, ``)}`, pubDate: r.indexOf(`-`) > 0 ? n(r) : t(r) };
            });
    return (
        (d = await Promise.all(
            d.map((t) =>
                e.tryGet(t.link, async () => {
                    let e = await r({ method: `get`, url: t.link }),
                        a = i(e.data),
                        o = e.data.match(/"publishedTime":"(.*)","publishedTimeString"/);
                    return (
                        (t.author = a(`.name`).text()),
                        (t.description = a(`#content`).html()),
                        (t.pubDate = o ? n(o[1]) : t.pubDate),
                        (t.category =
        ...
        { title: `${o ? u(`.department-header-active`).text() : `推荐`} -${s ? ` ${u(`.department-keywords-ul .active`).text()} -` : ``} MedSci.cn`, link: l, item: d }
    );
}
export { a as route };
