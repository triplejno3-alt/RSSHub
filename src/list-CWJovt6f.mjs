...
import { r, t as i } from './parse-article-6F1NcDY2.mjs';
const a = {
...
async function o(a) {
    let o = a.req.param(`user`),
        s = a.req.param(`catalogId`),
        c = e.medium.cookies[o],
        l = await r(o, s, c);
    if ((a.set(`json`, l), l && l.__typename === `Forbidden`)) throw new n(`无权访问 id 为 ${s} 的 List（可能是未设置 Cookie 或 Cookie 已过期）`);
    if (!l || !l.itemsConnection) throw new t(`id 为 ${s} 的 List 不存在`);
    let u = l.name,
        d = l.itemsConnection.items.map((e) => e.entity.mediumUrl),
        f = await Promise.all(d.map((e) => i(a, e)));
    return { title: `List: ${u}`, link: `https://medium.com/@${o}/list/${s}`, item: f };
}
export { a as route };
