...
const r = {
    ...
    handler: i,
    radar: [{ title: `Friend Works`, source: [`skeb.jp/:username`], target: `/friend_works/:username` }],
...
async function i(r) {
    ...
    let a = await n(i, `friend_works`);
    return { title: `Skeb - ${i} - フォロー中のクライアントの新着リクエスト`, link: `https://skeb.jp/${i}`, item: a };
}
export { r as route };
