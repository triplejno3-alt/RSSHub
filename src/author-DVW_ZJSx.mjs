...
async function a(i) {
    let a = i.req.param(`id`),
        o = `https://panewslab.com`,
        s = `${o}/webapi/subject/articles?Rn=${i.req.query(`limit`) ?? 25}&LId=1&tw=0&uid=${a}`,
        c = `${o}/zh/author/${a}.html`,
        l = (await n({ method: `get`, url: s })).data.data.map((e) => ({
            title: e.title,
            author: e.author.name,
            pubDate: t(e.publishTime * 1e3),
            link: `${o}/zh/articledetails/${e.id}.html`,
    ...
    return (
        ...
        { title: `PANews - ${l[0].author}`, link: c, item: l }
    );
}
export { i as route };
