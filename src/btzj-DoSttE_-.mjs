...
import { jsx as o } from 'hono/jsx/jsx-runtime';
import { load as s } from 'cheerio';
import { renderToString as c } from 'hono/jsx/dom/server';
const l = new Set([`2btjia.com`, `88btbtt.com`, `btbtt15.com`, `btbtt20.com`]),
    u = (e) => c(o(`table`, { children: e.map((e) => o(`tr`, { children: o(`td`, { children: e }) })) })),
    d = {
        path: `/:category?`,
...
async function f(o) {
    let c = o.req.param(`category`) ?? ``,
        d = o.req.query(`domain`) ?? `btbtt15.com`;
    if (!e.feature.allow_user_supply_unsafe_domain && !l.has(new URL(`http://${d}/`).hostname)) throw new a(`This RSS is disabled unless 'ALLOW_USER_SUPPLY_UNSAFE_DOMAIN' is set to 'true'.`);
    c === `base` ? ((c = ``), (d = `88btbtt.com`)) : c === `govern` && ((c = ``), (d = `2btjia.com`));
    let f = `https://www.${d}`,
        p = `${f}${c ? `/${c}.htm` : ``}`,
        m = s((await r({ method: `get`, url: p })).data);
    m(`.bg2`).prevAll(`table`).remove();
    let h = m(`#threadlist table`)
        .toArray()
        .map((e) => {
            let t = m(e).find(`.subject_link`);
            return { title: t.text(), link: `${f}/${t.attr(`href`)}` };
        });
    return (
        (h = await Promise.all(
            h.map((e) =>
                t.tryGet(e.link, async () => {
                    let t = s((await r({ method: `get`, url: e.link })).data);
                    (t(`h2, .message`).remove(),
                        t(`.attachlist`)
                            .find(`a`)
                            .each(function () {
                                (t(this)
                                    .children(`img`)
                                    .attr(`src`, `${f}${t(this).children(`img`).attr(`src`)}`),
                                    t(this).attr(
                                        `href`,
                                        `${f}/${t(this)
                                            .attr(`href`)
                    ...
                    let a = t(`.attachlist`).find(`a`);
                    return (
                        (e.description = t(`.post`).html()),
                        (e.author = t(`.purple, .grey`).first().prev().text()),
                        (e.pubDate = i(n(t(`.bg2 b`).first().text()), 8)),
                        a.length > 0 && ((e.description += u(a.toArray().map((e) => t(e).parent().html()))), (e.enclosure_type = `application/x-bittorrent`), (e.enclosure_url = a.first().attr(`href`))),
                        e
                    );
                })
            )
