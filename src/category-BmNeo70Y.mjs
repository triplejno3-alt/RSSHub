...
async function a(n) {
    let i = `https://bookfere.com/category/` + n.req.param(`category`),
        a = (await t({ method: `get`, url: i })).data,
        o = r(a),
        s = o(`main div div section`);
    return {
        title: o(`head title`).text(),
        link: i,
        item: s.toArray().map((t) => {
            t = o(t);
            let n = e(t.find(`time`).attr(`datetime`));
            return { title: t.find(`h2 a`).text(), link: t.find(`h2 a`).attr(`href`), pubDate: n, description: t.find(`p`).text() };
        }),
    };
}
export { i as route };
