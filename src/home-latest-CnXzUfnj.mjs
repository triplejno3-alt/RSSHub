...
const n = {
    ...
    handler: r,
    radar: [{ source: [`x.com/home`], target: `/home_latest` }],
};
async function r(n) {
    let { count: r, include_rts: i, only_media: a } = t.parseRouteParams(n.req.param(`routeParams`)),
        o = r ? { count: r } : {};
    await e.init();
    let s = await e.getHomeLatestTimeline(``, o);
    return (i || (s = t.excludeRetweet(s)), a && (s = t.keepOnlyMedia(s)), { title: `Twitter following timeline`, link: `https://x.com/home`, item: t.ProcessFeed(n, { data: s }) });
}
export { n as route };
