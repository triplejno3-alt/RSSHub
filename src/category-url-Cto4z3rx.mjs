...
import { i as r, r as i } from './utils-cMJuIYwm.mjs';
...
async function s(o) {
    let { language: s = `www`, url: c = `video` } = o.req.param(),
        l = `https://${s}.pornhub.com/${c}`;
    if (!n(s)) throw new t(`Invalid language`);
    let { data: u } = await e(l, { headers: i }),
        d = a(u),
        f = d(`#videoCategory .videoBox`)
            .toArray()
            .map((e) => r(d(e)));
    return { title: d(`title`).first().text(), link: l, language: d(`html`).attr(`lang`), item: f };
}
export { o as route };
