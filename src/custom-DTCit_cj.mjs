...
const s = {
    ...
    radar: [{ source: [`www.youtube.com/c/:id`], target: `/c/:id` }],
    name: `Custom URL`,
    maintainers: [`TonyRL`],
    handler: c,
};
async function c(s) {
    ...
    let c = s.req.param(`username`),
        l = !s.req.param(`embed`),
        { data: u } = await r(`https://www.youtube.com/c/${c}`),
        d = o(u),
        f = JSON.parse(
            d(`script`)
                .text()
        ...
        p = f.metadata.channelMetadataRenderer.externalId,
        m = (await a.getChannelWithId(p, `contentDetails`, t)).data.items[0].contentDetails.relatedPlaylists.uploads,
        h = (await a.getPlaylistItems(m, `snippet`, t)).data.items;
    return {
        title: `${c} - YouTube`,
        link: `https://www.youtube.com/c/${c}`,
        description: f.metadata.channelMetadataRenderer.description,
        image: f.metadata.channelMetadataRenderer.avatar?.thumbnails?.[0]?.url,
        item: h
            ...
            .map((e) => {
                ...
                return {
                    title: t.title,
                    description: a.renderDescription(l, r, i, a.formatDescription(t.description)),
