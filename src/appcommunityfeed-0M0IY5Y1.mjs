...
const a = {
    ...
    o = {
        ...
        parameters: { appid: `Steam appid, can be found on the community hub page or store page URL.`, routeParams: `Query parameters.` },
        radar: [
            { title: `Community Hub`, source: [`steamcommunity.com/app/:appid`], target: `/appcommunityfeed/:appid` },
            { title: `Community Hub`, source: [`store.steampowered.com/app/:appid/*/`], target: `/appcommunityfeed/:appid` },
        ],
...
`,
        ...
        handler: async (t) => {
            let { appid: r = 730, routeParams: o } = t.req.param(),
                c = await e(`https://steamcommunity.com/library/appcommunityfeed/${r}${o ? `?${o}` : ``}`);
            return {
                title: `${r} Steam Community Hub`,
                link: `https://steamcommunity.com/app/${r}`,
                item: c.hub.map((e) => ({
                    title: e.title === `` ? a[e.type] : e.title,
                    link: `https://steamcommunity.com/sharedfiles/filedetails/?id=${e.published_file_id}`,
                    description: i(n(s, { image: e.full_image_url, description: e.description })),
    ...
    s = ({ image: e, description: i }) => r(t, { children: [e ? n(`img`, { src: e }) : null, n(`p`, { children: i })] });
export { o as route };
