...
const i = async (i) => {
        let a = `http://due.hitsz.edu.cn`,
            o = {
                teaching: [`jwxw/jwgl`, `jwxw/kwgl`, `jwxw/zcgl`, `jwxw/xkgl`, `jwxw/cjgl`],
                studentStatus: [`jwxw/xjgl_b_`, `jwxw/xjgl_y_`],
                teachingSupport: [`jwxw/jxxxh`, `jwxw/jzxj`],
                education: [`xwgl/bksxw`, `xwgl/ssxwpy/ktyzj`, `xwgl/bsxwpy/qqhj1`],
            },
            { type: s = `all` } = i.req.param(),
            c = Object.keys(o).includes(s) ? s : `all`,
            l = c === `all` ? Object.values(o).flat() : o[c],
            u = l.map(async (e) => {
                let n = new URL(`${e}.htm`, a).href;
                try {
                    return await t(n);
                } catch {
                    return null;
        ...
        return {
            ...
            item: (await Promise.all(u))
                .flatMap((t, i) => {
                    if (!t) return [];
                    let o = l[i],
                        s = r(t.data);
                    return s(`ul.box-main-list li, .list-main li, .list-main-modular li`)
                        .toArray()
                        .map((t) => {
                            let r = s(t),
                                i = r.find(`a`).attr(`href`);
                            if (!i) return null;
                            let c = r.find(`span`).text().trim(),
                                l = r.find(`label`).text().trim();
                            return { title: c, link: new URL(i, a).href, pubDate: l ? n(e(l), 8) : null, category: o, description: c };
                        })
                        .filter(Boolean);
                })
                .filter(Boolean)
                .filter((e) => !e.title.includes(`统一身份认证平台`)),
...
- [博士学位培养](http://due.hitsz.edu.cn/xwgl/bsxwpy/qqhj1.htm)`,
        ...
        radar: [{ source: [`due.hitsz.edu.cn/jwxw/jwgl.htm`], target: `/hitsz/due/general` }],
    };
