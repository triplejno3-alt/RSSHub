...
const r = {
    ...
    features: {
        requireConfig: [{ name: `DISCOURSE_CONFIG_*`, description: `Configure the Discourse environment variables referring to [https://docs.rsshub.app/deploy/config#discourse](https://docs.rsshub.app/deploy/config#discourse).` }],
        requirePuppeteer: !1,
        antiCrawler: !1,
        supportBT: !1,
        supportPodcast: !1,
...
async function i(r) {
    let { link: i, key: a } = n(r),
        o = `${i}/${r.req.param(`path`)}.rss`,
        s = await t.parseString((await e(o, { headers: { 'User-Api-Key': a } })).data);
    return ((s.items = s.items.map((e) => ({ description: e.content, author: e.creator, ...e }))), { item: s.items, ...s });
}
export { r as route };
