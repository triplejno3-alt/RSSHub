...
async function o(a) {
    let o = `http://www.bjwxdxh.org.cn`,
        s = a.req.param(`type`),
        c = s ? `${o}/news/class/?${s}.html` : `${o}/news/class/`,
        l = i((await n({ method: `get`, url: c })).data),
        u = l(`div#newsquery > ul > li`)
            .toArray()
            .map((e) => ((e = l(e)), { title: e.find(`div.title > a`).text(), link: new URL(e.find(`div.title > a`).attr(`href`), o).href }));
    return (
        await Promise.all(
            u.map((a) =>
                e.tryGet(a.link, async () => {
                    ...
                    return (
                        (a.author = o[1]),
                        (a.pubDate = r(t(o[2], `YYYY-MM-DD HH:mm:ss`), 8)),
                        (a.description = e(`div#con`)
                            .html()
                            .trim()
                            .replaceAll(
                                `
