...
const a = async (t) => {
        let { category: a = `highlights`, country: o = `us` } = t.req.param(),
            s = Number.parseInt(t.req.query(`limit`) ?? `100`, 10),
            c = new URL(a.endsWith(`/`) ? a : `${a}/`, r).href,
            l = i(await e(c, { headers: { Cookie: `countryId=${o};` } })),
            u = l(`html`).attr(`lang`) ?? `en`,
            d = await n(l, `div.card-panel`, c, o, s),
            f = l(`title`).text();
        return {
            title: f,
        ...
        radar: [
            {
                ...
                target: (e) => {
                    let t = e.category;
                    return `/app-sales${t ? `/${t}` : ``}`;
                },
            },
            { title: `Highlights`, source: [`app-sales.net/highlights`], target: `/highlights` },
            { title: `Active Sales`, source: [`app-sales.net/activesales`], target: `/activesales` },
            { title: `Now Free`, source: [`app-sales.net/nowfree`], target: `/nowfree` },
        ],
        view: t.Articles,
    };
