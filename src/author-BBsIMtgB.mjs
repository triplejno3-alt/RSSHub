...
async function s(o) {
    let { domain: s = `91porn.com` } = o.req.query(),
        { uid: c, lang: l = `en_US` } = o.req.param(),
        u = `https://${s}/uvideos.php?UID=${c}&type=public`;
    i(s);
    let d = a((await n.post(u, { form: { session_language: l }, headers: { referer: u } })).data),
        f = d(`.row .well`)
            .toArray()
            .map((e) => ((e = d(e)), { title: e.find(`.video-title`).text(), link: e.find(`a`).attr(`href`), poster: e.find(`.img-responsive`).attr(`src`) }));
    return (
        (f = await Promise.all(
            f.map((i) =>
        ...
        { title: `${d(`.login_register_header`).text()} - 91porn`, link: u, item: f }
    );
}
export { o as route };
