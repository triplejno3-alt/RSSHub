...
async function s(o) {
    let s = o.req.param(`type`) ?? ``,
        ...
        l = o.req.param(`subCategory`) ?? ``,
        u = c === `` ? `` : `/${c}${l === `` ? `` : `/${l}`}`,
        d = `${`https://${s === `` ? `www` : `list`}.oilchem.net`}${s === `` ? `/1/` : s === `list` ? u : `/${a[`/${s}${u}`]}`}`,
        f = i((await n({ method: `get`, url: d })).data),
        p = f(`.list ul ul li a`)
            .toArray()
            .map((e) => ((e = f(e)), { title: e.text(), link: e.attr(`href`) })),
        m = await Promise.all(
            p.map((a) =>
                e.tryGet(a.link, async () => {
                    let e = i((await n({ method: `get`, url: a.link })).data);
                    return (
                        (a.description = e(`#content`).html()),
                        (a.pubDate = r(
                            t(
                                e(`.xq-head`)
                                    .find(`span`)
    ...
    return { title: `${f(`.hdbox h3`).text()} - 隆众资讯`, link: d, item: m };
}
export { o as route };
