...
const a = `https://www.1lou.me`,
    o = async (o) => {
        let { params: s } = o.req.param(),
            c = o.req.query(`limit`) ? Number.parseInt(o.req.query(`limit`), 10) : 50,
            l = Object.entries(o.req.query())
                .map(([e, t]) => `${encodeURIComponent(e)}=${encodeURIComponent(t)}`)
                .join(`&`),
            u = new URL(`${s && s.endsWith(`.htm`) ? s : `${s}.htm`}${l ? `?${l}` : ``}`, a).href,
            { data: d } = await n(u),
            f = i(d),
            p = f(`html`).prop(`lang`),
            m = f(`li.media.thread.tap:not(li.hidden-sm)`)
                .slice(0, c)
                .toArray()
                .map((e) => {
                    e = f(e);
                    let n = e.find(`div.subject`).children(`a`).first();
                    return {
                        title: n.text(),
                        pubDate: r(t(e.find(`span.date`).text()), 8),
                        link: new URL(n.prop(`href`), a).href,
                        category: [
                            e.find(`a.text-secondary`).text().replaceAll(`[]`, ``),
                            ...e
                                .find(`a.badge`)
                                .toArray()
                                .map((e) => f(e).text()),
                        ].filter(Boolean),
                        author: e.find(`a.username`).text(),
                        language: p,
                    };
                });
        m = await Promise.all(
            m.map((o) =>
                e.tryGet(o.link, async () => {
                    let { data: e } = await n(o.link),
                        s = i(e),
                        c = s(`h4.break-all`).contents().last().text();
                    if (c) {
                        let e = s(`div.message.break-all`).html(),
                            n = new URL(s(`img`).first().prop(`src`), a).href;
                        ((o.title = c),
                            (o.description = e),
                            (o.pubDate = r(t(s(`span.date`).text()), 8)),
                            (o.category = s(`a.badge`)
                                .toArray()
                                .map((e) => s(e).text())),
                            (o.content = { html: e, text: s(`div.message.break-all`).text() }),
                            (o.image = n),
                            (o.banner = n),
                            (o.language = p));
                        let i = s(`ul.attachlist li a`);
                        if (i.length > 0) {
                            let e = i.first();
                            ((o.enclosure_url = new URL(e.prop(`href`), a).href), (o.enclosure_type = `application/x-bittorrent`), (o.enclosure_title = e.text()));
                        }
                    }
                    return o;
                })
        ...
        let h = `BT 之家 1LOU 站`,
            g = new URL(f(`img.logo-2`).prop(`src`), a).href;
        return { title: `${f(`title`).text().split(/-/)[0]} - ${h}`, description: f(`meta[name="description"]`).prop(`content`), link: u, item: m, allowEmpty: !0, image: g, author: h, language: p };
    },
    s = {
        path: `/:params{.+}?`,
        name: `通用`,
        ...
        radar: [{ source: [`1lou.me/:params`], target: (e, t) => ((t = new URL(t)), `/1lou${t.href.replace(a, ``)}`) }],
    };
