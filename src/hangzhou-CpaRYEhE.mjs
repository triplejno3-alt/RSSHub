...
async function r() {
    let n = (await e({ method: `get`, url: `http://www.hzwgc.com/public/stop_the_water/` })).data,
        r = t(n),
        i = r(`.datalist li`);
    return {
        title: r(`title`).text(),
            ...
            .map(
                (e) => (
                    (e = r(e)),
                    {
                        ...
                        description: `杭州市停水通知：${e.find(`.title`).text()}`,
                        pubDate: new Date(e.find(`.published`).text()).toUTCString(),
                        link: `http://www.hzwgc.com${e.find(`.btn-read`).attr(`href`)}`,
                    }
                )
            ),
    };
