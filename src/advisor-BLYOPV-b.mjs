...
async function o(a) {
    let { category: o, type: s } = a.req.param(),
        c = i((await r({ method: `get`, url: `https://github.com/advisories?query=type%3A${s}+ecosystem%3A${o}` })).data),
        l = c(`div.Box-row`)
            .toArray()
            .map((e) => {
                e = c(e);
                let t = e.find(`a.Link--primary`),
                    r = e.find(`relative-time`).attr(`datetime`),
                    i = t.text() || `No title`,
                    a = t.attr(`href`) || `#`,
                    o = n(r || ``);
                return { title: i, link: `https://github.com${a}`, pubDate: o, description: `` };
            }),
        u = await Promise.all(
            l.map((n) =>
                t.tryGet(
    ...
    return { title: `GitHub Advisory Database RSS - ${o} - ${s}`, link: `https://github.com/advisories`, item: u };
}
export { a as route };
