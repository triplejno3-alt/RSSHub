...
const i = {
    ...
    radar: [{ source: [`hr.uibe.edu.cn/:category/:type`, `hr.uibe.edu.cn/:category`, `hr.uibe.edu.cn/`] }],
    name: `人力资源处`,
    maintainers: [`nczitzk`],
    handler: a,
    description:
...
async function a(i) {
    let a = i.req.param(`category`) ?? `tzgg`,
        o = i.req.param(`type`) ?? ``,
        s = `http://hr.uibe.edu.cn/${a}${o ? `/${o}` : ``}`,
        c = r((await n({ method: `get`, url: s })).data),
        l = c(`.lawul, .longul`)
            .find(`li a`)
            .toArray()
            .map((e) => ((e = c(e)), { title: e.find(`p`).text(), link: `${s}/${e.attr(`href`)}` }));
    return (
        (l = await Promise.all(
            l.map((i) =>
                e.tryGet(i.link, async () => {
                    try {
                        ...
                        ((i.description = e(`.gp-article`).html()), (i.pubDate = t(e(`#shareTitle`).next().text().replace(`时间：`, ``))));
                    } catch {
        ...
        { title: `${c(`.picTit`).text()} - 对外经济贸易大学人力资源处`, link: s, item: l }
    );
}
export { i as route };
