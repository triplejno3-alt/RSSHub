...
async function r(n) {
    let r = n.req.param(`type`),
        ...
        { data: a } = await e({ url: i[r][0] + `?page=1&limit=20&.me=c3lzLmluZGV4`, headers: { Referer: i[r][1] } });
    return {
        title: `本科生交换生管理系统-${i[r][2]}`,
        link: i[r][1],
        item:
            a &&
            a.data &&
            a.data.map((e) =>
                r === `proj`
                    ? { title: e.mc, description: e.mc, pubDate: t(e.cjsj), link: e.sqyq }
                    : r === `news`
                      ? { title: e.bt, author: e.createBy, description: e.nr, pubDate: t(e.createDate), link: `http://elite.nju.edu.cn/exchangesystem/index/detail?pid=${e.pid}` }
                      : null
            ),
    };
}
