...
const i = {
    ...
    radar: [{ source: [`tangshufang.com/:category`, `tangshufang.com/`] }],
    name: `分类`,
    maintainers: [`nczitzk`],
    handler: a,
...
async function a(i) {
    let a = i.req.param(`category`) ?? ``,
        ...
        s = `https://www.tangshufang.com${a ? `/${a}` : ``}`,
        ...
        l = c(`article`)
            ...
            .map((e) => {
                e = c(e);
                let n = e.find(`h2 a`);
                return { title: n.text(), link: n.attr(`href`), pubDate: t(e.find(`time`).text()) };
            });
    return (
        (l = await Promise.all(
            l.map((t) =>
                e.tryGet(t.link, async () => {
                    ...
                    return (
                        (t.description = e(`.wxsyncmain`).html()),
                        (t.category = e(`a[rel="category tag"]`)
                            .toArray()
                            .map((t) => e(t).text())),
                        t
                    );
