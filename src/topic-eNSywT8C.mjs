...
async function i(r) {
    let { data: i, url: a } = await t(`https://github.com/topics/${r.req.param(`name`)}`, { searchParams: new URLSearchParams(r.req.param(`qs`)) }),
        o = n(i);
    return {
        title: o(`title`).text(),
        description: o(`.markdown-body`).text().trim(),
        link: a,
        item: o(`article.my-4`)
            .toArray()
            .map((t) => {
                t = o(t);
                let n = t.find(`h3`).text().trim();
                return {
                    title: n,
                    author: n.split(`/`)[0],
                    description: (t.find(`a img`).prop(`outerHTML`) ?? ``) + t.find(`div > div > p`).text(),
                    link: `https://github.com${t.find(`h3 a`).last().attr(`href`)}`,
                    category: t
                        .find(`.topic-tag`)
                        .toArray()
                        .map((e) => o(e).text().trim()),
                    pubDate: e(t.find(`relative-time`).attr(`datetime`)),
                };
            }),
    };
}
