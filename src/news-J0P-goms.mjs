...
import { n as r } from './utils-L31uDd9Y.mjs';
var i = {
    competitionSeasonStageType: { competitionSeasonType: { competitionType: { leagueTypeName: `NBA`, competitionTypeName: `NBA` }, season: `2025-2026` }, competitionStageTypeName: `REGULAR` },
    rankTypeListMap: {
        E: [
            {
...
const a = [...i.rankTypeListMap.E, ...i.rankTypeListMap.W],
    o = {
        活塞: `Pistons`,
        尼克斯: `Knicks`,
        猛龙: `Raptors`,
    ...
    c = {
        ...
        maintainers: [`hyoban`],
        example: `/news/Spurs`,
        parameters: { team: { description: `全小写的英文队名，例如：spurs, lakers, warriors 等等` } },
        categories: [`bbs`],
        handler: async (i) => {
            let a = s[i.req.param(`team`)],
                o = a?.teamId;
            if (!o) throw Error(`Invalid team name`);
            let c = (await e(`https://games.mobileapi.hupu.com/3/7.5.60/basketballapi/news/v2/teamNewsById?cateGoryCode=basketball&clientId=93977196&newsId=0&teamId=${o}`)).result.map((e) => ({
                title: e.title,
                guid: e.tid,
                link: `https://m.hupu.com/bbs/${e.tid}`,
                pubDate: n(t(e.publishTime), 8),
            }));
            return ((c = await Promise.all(c.map((e) => r(e)))), { title: `虎扑 - ${a.teamName} 新闻`, link: `https://m.hupu.com`, item: c });
        },
    };
export { c as route };
