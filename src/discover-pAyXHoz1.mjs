import{t as e}from"./ofetch-uhy-qh6X.mjs";import"./config-Cc-zZ5p-.mjs";import"./logger-_vmdpChp.mjs";import"./parse-date-DjdQS_Nt.mjs";import"./timezone-CrV-DT8S.mjs";import{n as t,r as n,t as r}from"./util-BkASGVbN.mjs";import{load as i}from"cheerio";const a=async a=>{let{id:o}=a.req.param(),[s,c]=o?.split(/-/)??[void 0,void 0],l=Number.parseInt(a.req.query(`limit`)??`30`,10),u=new URL(`discover${o?`/${o}`:``}`,n).href,d=i(await e(u)),{apiRecommListUrl:f,apiRecommProcUrl:p,apiTagProcUrl:m}=await r(d),h,g,_=!!(s&&c);if(_){let t=((await e(f))?.data?.results??[]).find(e=>String(e.Id)===s),n=t?t.sublist.find(e=>String(e.Id)===c):void 0;h=t?.tag??t?.alias??void 0,g=n?.tag??n?.alias??void 0,_=!!(h&&g)}let v={page:1,pagesize:l,ticket:``},{data:{results:y}}=await(_?e(p,{query:{...v,ptag:h,stag:g}}):e(m,{query:{...v,f:`id`,o:`desc`}})),b=t(y?.slice(0,l)??[]),x=new URL(d(`img.logo`).prop(`src`),n).href,S=d(`title`).text().split(/_/).pop();return{title:`${S}${_?` | ${h} - ${g}`:``}`,description:d(`meta[property="og:description"]`).prop(`content`),link:u,item:b,allowEmpty:!0,image:x,author:S}},o={path:`/discover/:id?`,name:`发现`,url:`top.aibase.com`,maintainers:[`nczitzk`],handler:a,example:`/aibase/discover`,parameters:{id:`发现分类，默认为空，即全部产品，可在对应发现分类页 URL 中找到`},description:`::: tip
