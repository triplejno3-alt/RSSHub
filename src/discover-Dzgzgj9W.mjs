...
const u = ({ video: e, description: t }) => s(a(r, { children: [e ? i(`video`, { controls: !0, children: i(`source`, { src: e, type: `video/mp4` }) }) : null, t ? i(r, { children: c(t) }) : null] })),
    d = {
...
async function f(r) {
    let i = { 0: `精选`, 8: `平面`, 1: `插画`, 17: `UI`, 607: `网页`, 33: `摄影`, 24: `三维`, 610: `影视`, 609: `空间`, 499: `工业/产品`, 608: `动漫`, 612: `纯艺术`, 611: `手工艺`, 613: `服装`, 44: `其他` },
        a = {
        ...
        s = {
            cate: `0`,
            subCate: r.req.param(`subCate`) ?? `0`,
            city: r.req.param(`city`) ?? `0`,
            college: r.req.param(`college`) ?? `0`,
            recommendLevel: r.req.param(`recommendLevel`) ?? `2`,
            sort: r.req.param(`sort`) ?? `9`,
            limit: r.req.query(`limit`) ?? `25`,
        },
        c = r.req.param(`query`) ?? ``;
    if (c.includes(`=`)) {
        ...
        switch (c) {
            ...
            case `all`:
                ...
                break;
            case `article`:
                ((s.subCate = `809824`), (s.recommendLevel = `2`));
                break;
            case `editor`:
            case `edit`:
                s.recommendLevel = `2`;
                break;
            default:
                s.cate = r.req.param(`query`) ?? `0`;
        }
    for (let e in ((c = ``), s)) (e === `city` && (s[e] = s[e] in l ? l[s.city].toString() : (s[e] = `0`)), (c += `${e}=${s[e]}&`));
    c = c.slice(0, Math.max(0, c.length - 1));
    let d = `https://www.zcool.com.cn`,
        f = (await n({ method: `get`, url: `${d}/p1/discover/list?${c}`, headers: { Referer: d } })).data.datas.map((e) => ({
            title: e.content.title,
            link: e.content.pageUrl,
            pubDate: t(e.content.publishTime),
            author: e.content.creatorObj.username,
            category: e.content.tags,
        })),
        p = await Promise.all(
            f.map((t) =>
                e.tryGet(t.link, async () => {
                    let e = await n({ method: `get`, url: t.link }),
                        r = o(e.data);
                    r(`.prism-player, .image-info-icons`).remove();
                    let i = e.data.match(/source: '(https:\/\/video\.zcool\.cn\/.*)',/g);
                    return (
                        i &&
                            r(`.video-content-box`).each(function (e) {
                                e >= i.length || r(this).append(u({ video: i[e].match(/source: '(https:\/\/video\.zcool\.cn\/.*)'/)[1] }));
                            }),
                        (t.description = u({ description: t.link.includes(`article`) ? (r(`.articleContentWrapper`).html() ?? void 0) : (r(`.workShowBox`).parent().html() ?? void 0) })),
                        t
                    );
                })
            )
        );
    return { title: `站酷 - ${i[s.cate]}${s.subCate === `0` ? `` : ` - ${a[s.cate][s.subCate]}`}`, link: `${d}/discover?${c}`, item: p };
}
export { d as route };
