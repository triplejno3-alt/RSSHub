...
async function o(t) {
    let { name: a, id: o } = t.req.param(),
        s = `${a}のイベント・ライブ情報一覧`,
        c = `https://www.eventernote.com/actors/${a}/${o}/events`,
        l = Array.from({ length: 3 }, (e, t) => c + `?limit=20&page=${t + 1}`);
    return {
        title: s,
        link: c,
        description: s,
        language: `ja`,
        allowEmpty: !0,
        item: (await Promise.all(l.map((t) => e(t)))).flatMap((e) => {
            let t = n(e),
                a = t(`li.clearfix`);
            return a.length === 0
                ? []
                : a
                      .toArray()
                      .map((e) => {
                          let n = t(`div.event > h4 > a`, e).text(),
                              a = t(`div.event > div.place > a`, e).text(),
                              o = t(`div.date > p`, e).text().match(r),
                              s = o?.groups?.year,
                              c = o?.groups?.month,
                              l = o?.groups?.day,
                              u = t(`div.event > div.place > span.s`, e).text(),
                              d = { openHr: null, openMin: null, startHr: null, startMin: null, closeHr: null, closeMin: null };
                          for (let e of i) {
                              let t = u.match(e);
                              t !== null && (d = { ...d, ...t.groups });
                          }
                          let f = t(`div.event > h4 > a`, e).attr(`href`);
                          return {
                              title: n,
                              description: `イベント ${n}
開催場所 ${a}
