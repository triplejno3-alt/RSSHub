...
async function o(a) {
    let o = `http://www2.scut.edu.cn`,
        s = i[a.req.param(`category`) || `yjsjw`],
        c = `${o}/smae/${s.tag}/list.htm`,
        { data: l } = await n(c),
        u = r(l),
        d = u(`#wp_news_w6 ul li.news`)
            .toArray()
            .map((e) => {
                e = u(e);
                let n = e.find(`a`),
                    r = e.find(`span.news_meta`);
                return { title: n.attr(`title`), link: `${o}${n.attr(`href`)}`, pubDate: t(r.text()) };
            }),
        f = await Promise.all(
            d.map((t) =>
                e.tryGet(t.link, async () => {
                    let { data: e } = await n(t.link);
                    return ((t.description = r(e)(`div.wp_articlecontent`).html()), t);
                })
            )
        );
    return { title: `华南理工大学机械与汽车工程学院 - ${s.title}`, link: c, item: f };
}
export { a as route };
