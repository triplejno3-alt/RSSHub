...
async function a(i) {
    let { sort: a = `trending` } = i.req.param(),
        { posts: o } = await e(`https://huggingface.co/api/blog/community?sort=${a}`),
        s = o.map((e) => ({
            title: e.title,
            link: `https://huggingface.co${e.url}`,
            pubDate: n(e.publishedAt),
            author: e.authorsData?.[0]?.fullname || e.authorsData?.[0]?.name || `Unknown`,
            upvotes: e.upvotes,
