...
async function a(i) {
    let a = `http://www.crac.org.cn`,
        o = i.req.param(`type`),
        s = o ? `${a}/News/List?type=${o}` : `${a}/News/List`,
        c = r((await n({ method: `get`, url: s })).data),
        l = c(`div.InCont_r_d_cont > li`)
            .toArray()
            .map((e) => ((e = c(e)), { link: new URL(e.find(`a`).attr(`href`), a).href, pubDate: t(e.find(`span.cont_d`).text(), `YYYY-MM-DD`) }));
    return (
        await Promise.all(
            l.map((t) =>
                e.tryGet(t.link, async () => {
                    ...
                    return (
                        (t.title = e(`div.r_d_cont_title > h3`).text()),
                        (t.description = e(`div.r_d_cont`)
                            .html()
                            .trim()
                            .replaceAll(
                                `
