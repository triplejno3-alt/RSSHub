...
const i = {
    ...
    radar: [{ source: [`github.com/:user/:repo/stargazers`, `github.com/:user/:repo`] }],
    name: `Repo Stars`,
    maintainers: [`HenryQW`],
    handler: a,
};
async function a(n) {
    if (!e.github || !e.github.access_token) throw new r(`GitHub star RSS is disabled due to the lack of <a href="https://docs.rsshub.app/deploy/config#route-specific-configurations">relevant config</a>`);
    let i = n.req.param(`user`),
        a = n.req.param(`repo`),
        o = `https://github.com/${i}/${a}/stargazers`,
        s = (
            await t({
                method: `post`,
        ...
        ).data.data.repository.stargazers.edges.toReversed();
    return {
        allowEmpty: !0,
        title: `${i}/${a}’s stargazers`,
        link: o,
        item: s.map((e) => ({
            title: `${e.node.login} starred ${i}/${a}`,
            author: e.node.login,
