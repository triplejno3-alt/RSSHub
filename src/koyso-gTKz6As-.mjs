...
const s = (e) => o(i(r, { children: e?.map((e) => (e?.src ? i(`figure`, { children: i(`img`, { src: e.src, alt: e.alt ?? void 0 }) }) : null)) })),
    c = async (n) => {
        let { category: r = `0`, sort: i = `latest` } = n.req.param(),
            o = Number.parseInt(n.req.query(`limit`) ?? `30`, 10),
            c = `https://koyso.to`,
            l = new URL(`?${r === `0` ? `` : `category=${r}&`}sort=${i}`, c).href,
            u = a(await e(l)),
        ...
        ((f = u(`a.game_item`)
            ...
            .map((e) => {
                let t = u(e),
                    n = t.find(`div.game_info`).text(),
                    r = t.find(`div.game_media img`).attr(`data-src`),
                    i = s(r ? [{ src: r, alt: n }] : void 0),
                    a = t.attr(`href`);
                return { title: n, description: i, link: a ? new URL(a, c).href : void 0, content: { html: i, text: i }, image: r, banner: r, language: d };
            })),
            (f = await Promise.all(
                f.map((n) =>
                    n.link
                        ? t.tryGet(n.link, async () => {
                              ...
                              (t(`div.ind`).remove(), t(`div.download_div`).remove());
                              let r = t(`h1.content_title`).text();
                              t(`h1.content_title`).remove();
                              let i = n.description + (t(`div.game_content`).html() || ``),
                                  o = { title: r, description: i, content: { html: i, text: i }, language: d };
                              return { ...n, ...o };
                          })
                        : n
                )
            )));
        let p = u(`ul.category li#category_${r}`).text(),
            m = u(`div.genres_content ul li.${i}`).text();
        return { title: `${u(`title`).text()} - ${p} - ${m}`, description: u(`meta[name="description"]`).attr(`content`), link: l, item: f, allowEmpty: !0, language: d, id: l };
    },
    l = {
        path: `/:category?/:sort?`,
        name: `游戏`,
        ...
        radar: [
            {
                ...
                target: (e, t) => {
                    let n = new URL(t),
                        r = n.searchParams.get(`category`) ?? void 0,
                        i = n.searchParams.get(`sort`) ?? void 0;
                    return `/koyso${r ? `/${r}` : `0`}${i ? `/${i}` : ``}`;
                },
            },
            { title: `全部游戏`, source: [`koyso.to`], target: `/0` },
            { title: `动作游戏`, source: [`koyso.to`], target: `/3` },
            { title: `冒险游戏`, source: [`koyso.to`], target: `/5` },
            { title: `绅士游戏`, source: [`koyso.to`], target: `/7` },
            { title: `射击游戏`, source: [`koyso.to`], target: `/1` },
            { title: `休闲游戏`, source: [`koyso.to`], target: `/2` },
            { title: `体育竞速`, source: [`koyso.to`], target: `/4` },
            { title: `模拟经营`, source: [`koyso.to`], target: `/6` },
            { title: `角色扮演`, source: [`koyso.to`], target: `/8` },
            { title: `策略游戏`, source: [`koyso.to`], target: `/9` },
            { title: `格斗游戏`, source: [`koyso.to`], target: `/10` },
            { title: `恐怖游戏`, source: [`koyso.to`], target: `/11` },
            { title: `即时战略`, source: [`koyso.to`], target: `/12` },
            { title: `卡牌游戏`, source: [`koyso.to`], target: `/13` },
            { title: `独立游戏`, source: [`koyso.to`], target: `/14` },
            { title: `局域网联机`, source: [`koyso.to`], target: `/15` },
        ],
        view: n.Articles,
    };
