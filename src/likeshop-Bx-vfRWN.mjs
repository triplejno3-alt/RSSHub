import { t as e } from './ofetch-uhy-qh6X.mjs';
import './config-Cc-zZ5p-.mjs';
import './logger-_vmdpChp.mjs';
const t = {
    path: `/:site`,
    categories: [`social-media`],
    example: `/likeshop/bloombergpursuits`,
    parameters: { site: `the site attached to likeshop.me/` },
    radar: [{ source: [`likeshop.me/`] }],
    features: { requireConfig: !1, requirePuppeteer: !1, antiCrawler: !1, supportBT: !1, supportPodcast: !1, supportScihub: !1 },
    name: `Posts`,
    maintainers: [`nickyfoto`],
    handler: n,
    description: `LikeShop link in bio takes your audience from Instagram and TikTok to your website in one easy step.`,
};
async function n(t) {
    let n = t.req.param(`site`),
        r = (await e(`https://api.likeshop.me/api/accounts/${n}/galleries/likeshop`)).data.media.map((e) => ({
            title: e.comment,
            link: e.product_url.split(`?`)[0],
            description: `<p><img src="${e.image_url.split(`?`)[0]}"></p>`,
            guid: e.id,
        }));
    return { title: `@${n} Likeshop`, link: `https://likeshop.me/${n}`, item: r };
}
export { t as route };
