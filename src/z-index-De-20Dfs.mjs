...
const a = {
    ...
    radar: [{ source: [`agirls.aotter.net/posts/:category`], target: `/:category` }],
    name: `分类`,
    maintainers: [`TonyRL`],
    handler: o,
...
async function o(a) {
    let { category: o = `` } = a.req.param(),
        s = `${r}/posts${o ? `/${o}` : ``}`,
        c = i((await t(s)).data),
        l = c(`.ag-post-item__link`)
            .toArray()
            .map((e) => ((e = c(e)), { title: e.text().trim(), link: `${r}${e.attr(`href`)}` })),
        u = await Promise.all(l.map((t) => e.tryGet(t.link, () => n(t))));
    return { title: c(`head title`).text().trim(), link: s, description: c(`head meta[name=description]`).attr(`content`), item: u, language: c(`html`).attr(`lang`) };
}
export { a as route };
