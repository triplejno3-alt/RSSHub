...
async function c(s) {
    let c = s.req.param(`type`),
        l = !!s.req.param(`getDescription`) || !1,
        u = o.get(c).suffix,
        d = new URL(u, a).href,
        f = { headers: { cookie: await r(a) } },
        p = i((await n(d, f)).data),
        m = p(`#news_list ul li`)
            .slice(0, Math.min(Number.parseInt(p(`.per_count`, `#wp_paging_w6`).text()), Number.parseInt(p(`.all_count`, `#wp_paging_w6`).slice(1).text())))
            .toArray()
            .map((e) => ({ title: p(e).find(`a`).attr(`title`), link: p(e).find(`a`).attr(`href`), date: p(e).find(`span`).text() })),
        h = await Promise.all(
            m.map(async (r) => {
                let o = r.title || `tzgg`,
                    s = r.date,
                    c = new URL(r.link, a).href,
                    u = o + `<br><a href="` + c + `" target="_blank">查看原文</a>`;
                return (
                    l &&
                        (u = await e.tryGet(c, async () => {
                            let e = c.split(`.`).at(-1);
                            if (e === `htm` || e === `html`) return i((await n(c, f)).data)(`.wp_articlecontent`).html() + `<br><hr /><a href="` + c + `" target="_blank">查看原文</a>`;
                        })),
                    { title: o, link: c, description: u, pubDate: t(s) }
                );
            })
        );
    return { title: o.get(c).title, link: d, description: `南京航空航天大学计算机科学与技术学院RSS`, item: h };
}
export { s as route };
