...
async function r() {
    let {
        data: { objects: n },
    } = await t.get(`https://app.so/api/v5/appso/discount/?platform=web&limit=10`);
    return {
        title: `每日精品限免 / 促销应用`,
        link: `http://app.so/xianmian/`,
        description: `鲜面连线 by AppSo：每日精品限免 / 促销应用`,
        item: n.map((t) => ({
            title: `「${t.discount_info[0].discounted_price === `0.00` ? `免费` : `降价`}」${t.app.name}`,
            description: `
          ...
          ${t.content}
        `,
            pubDate: e(t.updated_at * 1e3),
            link: t.app.download_link[0].link,
        })),
    };
}
export { n as route };
